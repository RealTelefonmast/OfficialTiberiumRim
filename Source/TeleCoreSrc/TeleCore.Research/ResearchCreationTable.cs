...
public class ResearchCreationTable : IExposable
{
    ...
    {
        if (!tasksForThings.TryGetValue(thing.def, out var outList)) return;
        foreach (var task in outList)
        {
            taskCreations[task].AddCreation(thing);
            ResearchRoot.Research.CheckTask(task);
        }
    }

    public void TryTrackConstructedOrClaimedBuilding(ThingDef thingDef)
    {
        if (!tasksForThings.TryGetValue(thingDef, out var outList)) return;
        foreach (var task in outList)
        {
...
public class CreationGroupTracker : IExposable
{
    ...
    public void ExposeData()
    {
        ...
        if (Scribe.mode == LoadSaveMode.Saving) intValues = thingsToCreate.Values.ToList();
        Scribe_Collections.Look(ref intValues, "values");
        if (Scribe.mode == LoadSaveMode.PostLoadInit) Setup(taskDef, intValues);
    }
    ...
    {
        if (Completed || !creationOptionMap.TryGetValue(thingDef, out var options)) return;
        foreach (var option in options) AddProgress(option, 1);
    }

    public void AddCreation(Thing thing)
    {
        if (Completed || !creationOptionMap.TryGetValue(thing.def, out var options)) return;
    ...
    {
        thingsToCreate[optionProperties] =
            System.Math.Min(System.Math.Max(thingsToCreate[optionProperties] - progress, 0), optionProperties.amount);
    }
}