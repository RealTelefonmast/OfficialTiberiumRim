...
{
    private TResearchManager Manager => ResearchRoot.Research;
    ...
    {
        var currentProj = ResearchRoot.Research.CurrentProject;
        ...
        if (!PawnCapable(pawn, out var reason))
        {
            JobFailReason.Is($"\n{reason}");
            return false;
        }
    ...
    {
        reason = "";
        var canDoWork = pawn.workSettings.WorkIsActive(CurrentTask.WorkType);
        if (!canDoWork) reason += "TR_ResearchInactiveWorkType".Translate(CurrentTask.WorkType.labelShort) + "\n";
    ...
    {
        return t.Thing.GetStatValue(CurrentTask.RelevantPawnStat ?? StatDefOf.ResearchSpeedFactor);
    }
}