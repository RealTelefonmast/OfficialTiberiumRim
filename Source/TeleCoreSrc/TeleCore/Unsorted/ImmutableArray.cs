...
public readonly partial struct ImmutableArray<T> : IList<T>, IList, IStructuralComparable, IStructuralEquatable, IImmutableList<T>
{
    ...
    public ImmutableArray<T> InsertRange(int index, IEnumerable<T> items)
    {
        ...
        if (index != self.Length)
        {
            System.Array.Copy(self.array!, index, tmp, index + count, self.Length - index);
        }
    ...
    /// <summary>
    /// Copies the elements of the <see cref="ICollection"/> to an <see cref="System.Array"/>, starting at a particular <see cref="System.Array"/> index.
    /// </summary>
    /// <param name="array">The one-dimensional <see cref="System.Array"/> that is the destination of the elements copied from <see cref="ICollection"/>. The <see cref="System.Array"/> must have zero-based indexing.</param>
    ...
    {
        var self = this;
        System.Array? otherArray = other as System.Array;
        if (otherArray == null)
        {
    ...
    private ImmutableArray<T> InsertSpanRangeInternal(int index, ReadOnlySpan<T> items)
    {
        ...
        if (index != Length)
        {
            System.Array.Copy(array!, index, tmp, index + items.Length, Length - index);
        }
