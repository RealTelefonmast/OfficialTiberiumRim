...
public class SpriteSheetEditor : UIElement, IDragAndDropReceiver
{
    ...
    public void DrawHoveredData(object draggedObject, Vector2 pos)
    {
        if (draggedObject is WrappedTexture texture)
        {
            ...
            Verse.Widgets.DrawTextureFitted(CanvasRect, texture.Texture, 1);
            GUI.color = Color.white;

            Verse.Widgets.Label(new Rect(pos, size), label);
        }
    }
    ...
    private void DrawMouseOnGrid(Rect rect)
    {
        ...
        //Draw SnapPos
        Verse.Widgets.DrawLineHorizontal(mousePos.x - 5, mousePos.y, 10);
        Verse.Widgets.DrawLineVertical(mousePos.x, mousePos.y - 5, 10);
    ...
    private void DrawCanvas(Rect rect)
    {
        ...
        if (Texture == null) return;
        Verse.Widgets.DrawTextureFitted(rect, Texture, 1);
        ...
        Verse.Widgets.BeginGroup(rect);
        {
            for (var i = Tiles.Count - 1; i >= 0; i--)
            {
                ...
                Verse.Widgets.Label(sRect.ContractedBy(1), $"[{i}]");

                //
                if (Verse.Widgets.CloseButtonFor(sRect))
                {
                    Tiles.RemoveAt(i);
                    if (selTile != null)
            ...
            if (Verse.Widgets.ButtonImage(new Rect(rect.width - 25, rect.height - 25, 25, 25), TeleContent.LockOpen,
                    false))
                Clear();
        }
    ...
    {
        Verse.Widgets.BeginGroup(CanvasRect);
        {
    ...
    private void DrawTileList(Rect rect)
    {
        ...
        Verse.Widgets.BeginScrollView(rect, ref spriteListScrollPos, fullRect, false);
        float curY = 0;
        for (var i = 0; i < Tiles.Count; i++)
        {
            ...
            Verse.Widgets.Label(tileRect, $"[{i}]");
            curY += _ListSize.y;
            if (Verse.Widgets.ButtonInvisible(tileRect))
            {
                selTile = tile;
                selIndex = i;
            }
    ...
    {
        Verse.Widgets.DrawLine(new Vector2(rect.x, rect.y - 10f), new Vector2(rect.xMax, rect.y - 10), TColor.White025, 1);
        Verse.Widgets.DrawBoxSolid(TileOutPutRect, TColor.BGDarker);
        ...
        for (var i = 0; i < Tiles.Count; i++)
        {
            ...
            if (sFlag)
                Verse.Widgets.DrawHighlight(spriteRect);
            ...
            if (Verse.Widgets.ButtonInvisible(spriteRect))
            {
                selTile = tile;
                selIndex = i;
            }
