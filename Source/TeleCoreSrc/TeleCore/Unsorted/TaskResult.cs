...
public struct ResultState<T>
{
    ...
    public static implicit operator ResultState<T>(T value) => new ResultState<T> { Result = value, State = "Default" };
}
...
public class TaskResult<T> : TaskResult, ITask<T, ResultState<T>>
{
    ...
    public ITaskAwaiter<ResultState<T>> GetAwaiter() => new TaskResultAwaiter<T>(this);
