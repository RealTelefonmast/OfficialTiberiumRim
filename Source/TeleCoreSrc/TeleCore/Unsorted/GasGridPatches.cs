...
public static class GasGridPatches
{
    ...
    {
        private static MethodBase ReadoutCall = AccessTools.Method(typeof(DrawUtils), nameof(DrawUtils.TryDrawExtraGasGrid));
        ...
        public static void TryDrawExtraGasGrid(ref float curYOffset)
        {
            ...
            if (minimalGasGrid.AnyGasAt(intVec))
            {
                Widgets.Label(
                    ...
                    "This is where you'd see gas if it were done!" + " " + ((float)69 / 255f).ToStringPercent("F0"));
                curYOffset += 19f;
            }
        }
    }
