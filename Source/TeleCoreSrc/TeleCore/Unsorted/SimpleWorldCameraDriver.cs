...
public class SimpleWorldCameraDriver : MonoBehaviour
{
    ...
    private Vector2 CalculateCurInputDollyVect()
    {
        ...
        {
            var mousePositionOnUI = Verse.UI.MousePositionOnUI;
            var mousePositionOnUIInverted = Verse.UI.MousePositionOnUIInverted;
            var rect = new Rect(Verse.UI.screenWidth - 250, 0f, 255f, 255f);
            var rect2 = new Rect(0f, Verse.UI.screenHeight - 250, 225f, 255f);
            var rect3 = new Rect(Verse.UI.screenWidth - 250, Verse.UI.screenHeight - 250, 255f, 255f);
            ...
            if (Find.WindowStack.IsOpen<WorldInspectPane>() && inspectPane.RecentHeight > rect2.height)
                rect2.yMin = Verse.UI.screenHeight - inspectPane.RecentHeight;
                ...
                !rect.Contains(mousePositionOnUIInverted))
            {
                ...
                if (mousePositionOnUI.x <= Verse.UI.screenWidth && mousePositionOnUI.x > Verse.UI.screenWidth - 20f)
                    zero.x += config.dollyRateScreenEdge;
                if (mousePositionOnUI.y <= Verse.UI.screenHeight && mousePositionOnUI.y > Verse.UI.screenHeight - 20f)
    ...
    private float CurUITileSize(Camera worldCam, float avarageTileSize)
    {
        var WorldToUI = delegate(Vector3 worldLoc)
        {
            ...
            return new Vector2(vector.x, Verse.UI.screenHeight - vector.y);
        };
