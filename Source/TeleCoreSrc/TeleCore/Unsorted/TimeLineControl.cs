...
internal class TimeLineControl : UIElement
{
    ...
    protected override void DrawContentsBeforeRelations(Rect inRect)
    {
        ...
        //
        float curY = 0;
        Verse.Widgets.BeginScrollView(BotLeft, ref elementScrollPos, elementListViewRect, false);
        {
        ...
        //
        Verse.Widgets.DrawBoxSolid(RightRect, TColor.BGDarker);
        Verse.Widgets.ScrollHorizontal(RightRect, ref timeLineScrollPos, timelineViewRect);
        Verse.Widgets.BeginScrollView(RightRect, ref timeLineScrollPos, timelineViewRect, false);
        {
    ...
    private void DrawEventFlags(Rect rect, float height)
    {
        ...
        foreach (var eventFlag in CurrentPart.InternalEventFlags)
        {
            ...
            Verse.Widgets.DrawLineVertical(valX, selectorRect.yMax - 5, height);
            GUI.color = Color.white;
    ...
    private void DrawTimeSelector(Rect timeBar, float totalHeight)
    {
        ...
        Verse.Widgets.DrawLineHorizontal(timeBar.x, timeBar.yMax, timeBar.width);
        GUI.color = Color.white;
        ...
        for (var i = 0; i <= AnimationLength; i++)
        {
            ...
            Verse.Widgets.DrawLineVertical(pos.x, pos.y, length);
            if (bigOne)
            {
                var label = $"{i / 60}s";
                pos -= new Vector2(0, 4);
        ...
        int min = 0, int max = 100, int minWidth = 0)
    {
        ...
        */
        ...
        Verse.Widgets.DrawLineVertical(valX, selectorRect.yMax - 5, verticalHeight);

        if (Verse.Widgets.curDragEnd == Verse.Widgets.RangeEnd.None && (Verse.Widgets.draggingId == id || Mouse.IsOver(selectorRect)))
        ...
        if (Verse.Widgets.curDragEnd == Verse.Widgets.RangeEnd.Min || Mouse.IsOver(rangeLBar))
            Verse.Widgets.DrawHighlight(rangeLBar);
        if (Verse.Widgets.curDragEnd == Verse.Widgets.RangeEnd.Max || Mouse.IsOver(rangeRBar))
            Verse.Widgets.DrawHighlight(rangeRBar);
        ...
        {
            if (mouseUp)
                Verse.Widgets.curDragEnd = Verse.Widgets.RangeEnd.None;
            Verse.Widgets.draggingId = 0;
            SoundDefOf.DragSlider.PlayOneShotOnCamera();
        }

        var flag = false;
        if (Mouse.IsOver(rect) || Verse.Widgets.draggingId == id)
        {
            if (Event.current.type == EventType.MouseDown && Event.current.button == 0 && id != Verse.Widgets.draggingId)
            {
                Verse.Widgets.draggingId = id;
                var x = Event.current.mousePosition.x;
                if (x >= rangeLPos.xMin && x <= rangeLPos.xMax)
                    Verse.Widgets.curDragEnd = Verse.Widgets.RangeEnd.Min;
                else if (x >= rangeRPos.xMin && x <= rangeRPos.xMax)
                    Verse.Widgets.curDragEnd = Verse.Widgets.RangeEnd.Max;
                else
                    Verse.Widgets.curDragEnd = Verse.Widgets.RangeEnd.None;
                /*
            ...
            if (flag || Event.current.type == EventType.MouseDrag)
            {
                ...
                //Value Selection
                if (Verse.Widgets.curDragEnd == Verse.Widgets.RangeEnd.None)
                {
                    var newSliderVal =
                ...
                //Range selection
                if (Verse.Widgets.curDragEnd == Verse.Widgets.RangeEnd.Min)
                {
                    if (num5 != range.min)
                    {
                        range.min = num5;
                ...
                else if (Verse.Widgets.curDragEnd == Verse.Widgets.RangeEnd.Max && num5 != range.max)
                {
                    range.max = num5;
                ...
                Verse.Widgets.CheckPlayDragSliderSound();
                Event.current.Use();
            }
        }
    }
    ...
    private void ElementListing(Rect leftRect, IKeyFramedElement element)
    {
        ...
        Verse.Widgets.Label(leftRect, $"{((TextureElement) element).LayerTag}");
        if (Verse.Widgets.ButtonImage(leftRect.RightPartPixels(20), TeleContent.DeleteX))
    ...
    {
        Verse.Widgets.DrawHighlightIfMouseover(rightRect);
        GUI.color = SelectedElement == element ? Color.white : TColor.White025;
        var yPos = rightRect.y + rightRect.height / 2f;
        Verse.Widgets.DrawLineHorizontal(rightRect.x, yPos, rightRect.width);
        GUI.color = Color.white;
        ...
        for (var i = 0; i < elements.Count; i++)
        {
            ...
            if (draggedKeyframe.HasValue)
                if (draggedKeyframe.Value.Item1 == element && draggedKeyframe.Value.Item2 == keyFrame)
                {
                    ...
                    Verse.Widgets.DrawTextureFitted(rect2, TeleContent.KeyFrame, 1f);
                    GUI.color = Color.white;
                }
            ...
            Verse.Widgets.DrawTextureFitted(rect, TeleContent.KeyFrame, 1f);
            if (isSelected) Verse.Widgets.DrawTextureFitted(rect, TeleContent.KeyFrameSelection, 1f);
    ...
    {
        Verse.Widgets.BeginGroup(topPart);
        var row = new Verse.WidgetRow();
