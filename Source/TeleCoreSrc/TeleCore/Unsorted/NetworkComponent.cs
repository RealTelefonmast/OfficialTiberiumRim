...
namespace TeleCore.Unsorted
{
    public class NetworkComponent : IExposable, INetworkComponent, IContainerHolderStructure
    {
        ...
        private void ProcessValues()
        {
            ...
            if (NetworkRole.HasFlag(this.NetworkRole.Producer))
            {
                ProducerTick();
            }

            //Storages push to Consumers
            if (NetworkRole.HasFlag(this.NetworkRole.Storage))
            {
                StorageTick();
            }

            //Consumers slowly use up own container
            if (NetworkRole.HasFlag(this.NetworkRole.Consumer))
            {
                ConsumerTick();
            }
        ...
        protected virtual void StorageTick()
        {
            ...
            {
                TransferToOthers(this.NetworkRole.Storage, this.NetworkRole.Storage, true);
            }
        ...
        protected virtual void RequesterTick()
        {
            if (RequesterMode == RequesterMode.Automatic)
            {
                ...
                {
                    var valTypeValue = Container.ValueForType(valType) + Network.NetworkValueFor(valType, this.NetworkRole.Storage);
                    if (valTypeValue > 0)
                    {
