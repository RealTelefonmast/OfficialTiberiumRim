...
public class OxygenBurnerFire : AtmosphereConverter
{
    ...
    public override void Tick()
    {
        ...
        {
            foreach (var atmos in Atmospheres) BurnOxygen(atmos);
            return;
        }

        if (Atmosphere == null)
        {
            Log.WarningOnce($"Tried to tick oxygen burner with thing without a room: {_sourceThing}",
                _sourceFire.GetHashCode());
            return;
        }
