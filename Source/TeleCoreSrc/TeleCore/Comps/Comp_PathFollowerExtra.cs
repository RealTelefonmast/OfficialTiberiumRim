...
public class Comp_PathFollowerExtra : ThingComp
{
    ...
    private void UpdateRoomsOnPath()
    {
        ...
        for (var i = 0; i < pathNodes.Count; i++)
        {
            ...
            if (newRoom == null)
            {
                TLog.Warning($"{Pawn.NameFullColored} has path node with null room.");
                continue;
            }
            ...
            //Predict:
            for (var k = i; k < pathNodes.Count; k++)
            {
                var nextNode = pathNodes[pathNodes.Count - 1 - k];
                var nextNewRoom = nextNode.GetRoom(Pawn.Map);
                if (nextNewRoom == null)
                {
                    TLog.Warning($"{Pawn.NameFullColored} has predicted path node with null room.");
                    continue;
                }
