...
public class CompProperties_PawnExtraDrawer : Verse.CompProperties
{
...
{
    //[0] = Body | [1] = Head
    public Dictionary<string, Graphic[]> drawGraphics = new();
    public Dictionary<string, GraphicData[]> graphicData = new();
    ...
    private void DrawOntoHead(Graphic head, Vector3 drawPos, Quaternion rot, bool renderBody, Rot4 bodyRot,
        Rot4 headRot, bool portrait, bool headStump, bool mirrored)
    {
    }
    ...
    {
        if (rotation == Rot4.North || rotation == Rot4.South) return true;
        return mirrored ? rotation == Rot4.East : rotation == Rot4.West;
    }

    private bool ShouldMirror(Rot4 rotation, bool mirrored)
    {
        if (rotation == Rot4.North || rotation == Rot4.South) return mirrored;
        return false;
    }

    private void DrawOntoHuman(Pawn pawn, Graphic head, Graphic body, Vector3 drawPos, Quaternion rot, bool renderBody,
        Rot4 bodyRot, Rot4 headRot, bool portrait, bool headStump, bool mirrored)
    {
        var drawVector = drawPos;
        if (head != null && !headStump && ShouldShow(headRot, mirrored))
        {
            ...
            var b = rot * pawn.Drawer.renderer.BaseHeadOffsetAt(headRot);
    ...
    public void DrawExtraLayers(Pawn pawn, Vector3 drawPos, Quaternion rotation, bool renderBody, Rot4 bodyFacing,
        Rot4 headFacing, bool portrait, bool headStump)
    {
        foreach (var graphicPair in graphicData)
        {
            ...
            */
            ...
            var drawMirrored = graphicPair.Key.Contains("_Mirror");
            var head = graphicData[1].Graphic;
            var body = graphicData[0].Graphic;

            if (!pawn.def.race.Humanlike)
                DrawOntoAnimal(pawn, body, drawPos, rotation, bodyFacing);
            else
                DrawOntoHuman(pawn, head, body, drawPos, rotation, renderBody, bodyFacing, headFacing, portrait,
                    headStump, drawMirrored);
        }
    }
