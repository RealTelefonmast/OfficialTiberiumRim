...
internal class Dialog_AnimationFileList : Dialog_FileList
{
    ...
    public override void DoWindowContents(Rect inRect)
    {
        ...
        var row = new Verse.WidgetRow(animRow.x, animRow.y, gap: 0);
        ...
        //Animation Def
        var row2 = new Verse.WidgetRow(defRow.x, defRow.y, gap: 0);
    ...
    private void TrySaveAnimationDef()
    {
        try
        {
            AnimationSaveUtility.CreateAnimationDef($"{Context.defName}Def", "Defs", delegate
            {
                ...
                Scribe_Deep.Look(ref newDef, $"{nameof(Static.TeleCore)}.{nameof(AnimationDataDef)}");
            });
        }
        catch (Exception arg)
        {
