...
public class TResearchTaskDef : Def
{
    ...
    [Unsaved] private ResearchWorker workerInt;
    public WorkTypeDef workType;

    public ResearchWorker Worker
    {
        get { return workerInt ??= (ResearchWorker)Activator.CreateInstance(researchWorker); }
    }
    ...
    public TResearchDef ParentProject
    {
        get
        {
            return parentProject ??=
                DefDatabase<TResearchDef>.AllDefsListForReading.FirstOrDefault(r =>
                    !r.tasks.NullOrEmpty() && r.tasks.Contains(this));
        }
    }

    [field: Unsaved] public List<ThingDef> UnlocksThings { get; } = new();
    ...
    public List<ThingDef> PossibleMainTargets
    {
        get
        {
            ...
            if (mainTargets == null)
            {
                ...
                mainTargets =
                    DefDatabase<ThingDef>.AllDefsListForReading.FindAll(t =>
    ...
    public virtual float ProgressReal => creationTasks != null
        ? ResearchRoot.ResearchCreationTable().TaskCompletion(this)
        : ResearchRoot.Research.GetProgress(this);
    ...
    {
        return GenRadial.RadialCellsAround(origin, distanceFromTarget - distanceRange / 2,
            distanceFromTarget + distanceRange / 2);
    }
    ...
    public void TriggerEvents()
    {
        ...
        foreach (var @event in events) TRUtils.EventManager().StartEvent(@event);
    }
    ...
    public void Debug_Finish()
    {
        ...
        ResearchRoot.Research.SetProgress(this, ProgressToDo);
    }

    //[SyncMethod(SyncContext.None)]
    public void Debug_Reset()
    {
        ResearchRoot.Research.SetProgress(this, 0);
        ResearchRoot.Research.SetCompleted(this, false);
        ResearchRoot.Research.ResearchCompleted.Remove(ParentProject);
        foreach (var eventDef in events) TRUtils.EventManager().ResetEvent(eventDef);

        if (creationTasks != null)
            foreach (var option in creationTasks.thingsToCreate)
                ResearchRoot.ResearchCreationTable().taskCreations[this].AddProgress(option, -option.amount);
    }
    ...
    public void WriteTaskInfo(Rect rect, out float height)
    {
        ...
        if (CachedInfo.TryGetValue("TargetProps_Label", out var targetLabel))
        {
            ...
            //Targets
            if (CachedInfo.TryGetValue("TargetProps_Targets", out var values))
        ...
        if (CachedInfo.TryGetValue("CreationTasks_Label", out var taskLabel))
        {
            ...
            //Tasks
            if (CachedInfo.TryGetValue("CreationTasks_Targets", out var values))
                for (var i = 0; i < values.Length; i++)
                {
                    var targetHeight = Text.CalcHeight(values[i], Width);
                    var targetRect = new Rect(0, curY, Width, targetHeight);
        ...
        //WorkType
        if (CachedInfo.TryGetValue("WorkType_Label", out var workTypeLabel))
        {
            //Target Label
        ...
        if (CachedInfo.TryGetValue("SkillReq_Label", out var skillLabel))
        {
            ...
            //Skills
            if (CachedInfo.TryGetValue("SkillReq_Skills", out var values))
                for (var i = 0; i < values.Length; i++)
                {
                    var targetHeight = Text.CalcHeight(values[i], Width);
                    var targetRect = new Rect(0, curY, Width, targetHeight);
                    ...
                    if (mouseOver) pawns.ForEach(p => TargetHighlighter.Highlight(p, false));
    ...
    private void DoMenuOptions(Rect rect, ThingDef def, bool allowJump)
    {
        ...
        var row = new WidgetRow(rect.xMax, rect.y, UIDirection.LeftThenDown, 9999f, 0);
        ...
        if (def.IsConstructible() && row.ButtonIcon(TRContent.Construct, ""))
        {
            ...
            if (def.IsResearchFinished)
            {
                ...
                Architect.selectedDesPanel.def.ResolvedAllowedDesignators
                    .First(t => t is Designator_Build b && b.entDef == def).ProcessInput(null);
                return;
            }
        }
    ...
    public void CacheTaskInfo()
    {
        ...
        //WorkType
        CachedInfo.Add("WorkType_Label",
            new[] { "TR_TaskWorkType".Translate(WorkType.labelShort.ColorizeFix("#00C8CC")).RawText });
    ...
    public List<Pawn> CapablePawns()
    {
        ...
        foreach (var skillRequirement in skillRequirements) pawns.AddRange(CapablePawnsFor(skillRequirement));
