...
public class Building_Airlock : Building_Door
{
    ...
    public RoomComponent_AirLock[] RoomComps { get; private set; }

    //Main Conditions
    public bool IsFunctioning => RoomComps[0] != null && RoomComps[1] != null;
    ...
    {
        Rooms = new Room[2];
        RoomComps = new RoomComponent_AirLock[2];
    ...
    public void SetAirlock(RoomComponent_AirLock airlock)
    {
        ...
        if (RoomComps[1] == null) SetAirlock(1, airlock);
    }
    ...
    {
        if (RoomComps[index].LockedDown) return !RoomComps[index].Parent.ContainsPawn(forPawn);
        if (!RoomComps[index].IsClean) return !OppositeRoomComp(index).CanVent;
        return false;
    }
    ...
    public override string GetInspectString()
    {
        ...
        if (DebugSettings.godMode)
        {
            ...
            if (IsFunctioning)
            {
                if (RoomComps[0].LockedDown) sb.AppendLine($"[WARNING LOCKDOWN][0][{Rooms[0].ID}]".Colorize(Color.red));

                if (RoomComps[1].LockedDown) sb.AppendLine($"[WARNING LOCKDOWN][1][{Rooms[1].ID}]".Colorize(Color.red));
            }
        }
    ...
    public override void DrawAt(Vector3 drawLoc, bool flip = false)
    {
        ...
        if (Find.Selector.IsSelected(this))
        {
            ...
            if (RoomComps[0] != null) GenDraw.DrawFieldEdges(RoomComps[0].Room.Cells.ToList(), Color.cyan);
            if (RoomComps[1] != null) GenDraw.DrawFieldEdges(RoomComps[1].Room.Cells.ToList(), Color.magenta);
        }
    }
}