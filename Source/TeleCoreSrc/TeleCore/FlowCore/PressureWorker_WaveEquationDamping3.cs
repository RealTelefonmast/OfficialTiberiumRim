...
public class PressureWorker_WaveEquationDamping3 : PressureWorker
{
    ...
    public override DefValueStack<NetworkValueDef, double> FlowFunction(
        FlowInterface<NetworkPart, NetworkVolume, NetworkValueDef> iface, DefValueStack<NetworkValueDef, double> f)
    {
        ...
        if (iface.Mode == InterfaceFlowMode.FromTo && dp <= 0) return DefValueStack<NetworkValueDef, double>.Empty;
