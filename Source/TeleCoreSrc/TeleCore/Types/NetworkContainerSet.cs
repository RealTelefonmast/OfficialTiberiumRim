...
public class NetworkContainerSet
{
    ...
    private readonly Dictionary<NetworkRole, Dictionary<NetworkValueDef, float>> ValueByTypeByRole = new();
    private Color networkColor = new(0, 0, 0, 0);

    public NetworkContainerSet()
    {
        //
        ContainersByRole = new Dictionary<NetworkRole, HashSet<NetworkContainer>>();
        ...
        //
        TotalValueByRole = new Dictionary<NetworkRole, float>();
    ...
    public float GetTotalValueByRole(NetworkRole role)
    {
        ...
        float totalVal = 0;
        foreach (var @enum in role.AllFlags()) totalVal += TotalValueByRole.GetValueOrDefault(@enum, 0);
        return totalVal;
    }
    ...
    public void Notify_RemovedValue(NetworkValueDef type, float value, INetworkComponent comp)
    {
        ...
        foreach (var enums in comp.NetworkRole.AllFlags())
        {
            if (!TotalValueByRole.ContainsKey(enums))
                TLog.Warning(
                    $"Tried to remove ({value}){type} for role {enums} in ContainerSet but {enums} is not stored!");
            else
                TotalValueByRole[enums] -= value;
    ...
    private void AddContainerFrom(INetworkComponent component, NetworkContainer container)
    {
        foreach (var @enum in component.NetworkRole.AllFlags())
        {
            if (!ContainersByRole.ContainsKey(@enum))
                ContainersByRole.Add(@enum, new HashSet<NetworkContainer>());
            ContainersByRole[@enum].Add(container);
        }

        //Adjust values
        foreach (var values in container.StoredValuesByType) Notify_AddedValue(values.Key, values.Value, component);
    }
    ...
    private void RemoveContainerFrom(INetworkComponent component, NetworkContainer container)
    {
        ...
        foreach (var @enum in component.NetworkRole.AllFlags()) this[@enum].Remove(container);

        //Adjust values
        foreach (var values in container.StoredValuesByType) Notify_RemovedValue(values.Key, values.Value, component);
    }
}