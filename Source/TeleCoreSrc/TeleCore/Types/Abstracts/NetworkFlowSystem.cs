...
public abstract class NetworkFlowSystem : FlowSystem<NetworkPart, NetworkVolume, NetworkValueDef>
{
    ...
    {
        if (part.CachedVolume != null) return part.CachedVolume;
    ...
    {
        if (part.Config.volumeConfig != null) GenerateForOrGetVolume(part);
    }
    ...
    internal void Notify_Populate(NetworkGraph graph)
    {
        foreach (var edgePair in graph.EdgesByNodes)
        {
            ...
            var mode = edge.BiDirectional ? InterfaceFlowMode.TwoWay : InterfaceFlowMode.FromTo;
            var iFace = new FlowInterface<NetworkPart, NetworkVolume, NetworkValueDef>(edge.From, edge.To, fb1, fb2,
                mode);
            AddInterface((edge.From, edge.To), iFace);
        }
    }
    ...
    protected override DefValueStack<NetworkValueDef, double> FlowFunc(
        FlowInterface<NetworkPart, NetworkVolume, NetworkValueDef> connection,
        DefValueStack<NetworkValueDef, double> previous)
    {
        return PressureWorker.FlowFunction(connection, previous);
    }

    protected override DefValueStack<NetworkValueDef, double> ClampFunc(
        FlowInterface<NetworkPart, NetworkVolume, NetworkValueDef> iface, DefValueStack<NetworkValueDef, double> flow,
        ClampType clampType)
    {
