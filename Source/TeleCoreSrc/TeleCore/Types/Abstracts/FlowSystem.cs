    ...
    where TVolume : FlowVolumeBase<TValueDef>
{
    ...
    public IReadOnlyDictionary<TVolume, HashSet<FlowInterface<TAttach, TVolume, TValueDef>>> Connections =>
        _connections;

    public IReadOnlyDictionary<TwoWayKey<TAttach>, FlowInterface<TAttach, TVolume, TValueDef>> InterfaceLookUp =>
        _interfaceLookUp;
    ...
    protected void RemoveInterface(TwoWayKey<TAttach> connectors)
    {
        if (_interfaceLookUp.TryGetValue(connectors, out var iFace))
        {
            ...
            if (_connections.TryGetValue(iFace.From, out var conns)) conns.Remove(iFace);
        }
    }
    ...
    protected void RemoveInterfacesWhere(Predicate<FlowInterface<TAttach, TVolume, TValueDef>> predicate)
    {
        for (var i = _interfaces.Count - 1; i >= 0; i--)
        {
            ...
            if (predicate.Invoke(iFace))
            {
                ...
                if (_connections.TryGetValue(iFace.From, out var conns)) conns.Remove(iFace);
                if (_connections.TryGetValue(iFace.To, out var conns2)) conns2.Remove(iFace);
            }
        }
    }
    ...
    {
        if (_relations.TryAdd(key, volume)) return true;
