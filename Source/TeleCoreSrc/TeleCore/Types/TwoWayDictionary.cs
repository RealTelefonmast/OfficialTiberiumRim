    ...
    IEnumerable<KeyValuePair<TKey, TValue>>
{
    ...
    private readonly Dictionary<TValue, List<TKey>> _valueToKey;

    public TwoWayDictionary()
    {
        _keyToValue = new Dictionary<TKey, TValue>();
        _valueToKey = new Dictionary<TValue, List<TKey>>();
    }
    ...
    {
        if (_keyToValue.ContainsKey(key)) return false;

        _keyToValue.Add(key, value);
        if (!_valueToKey.ContainsKey(value)) _valueToKey.Add(value, new List<TKey>());
    ...
    {
        if (!_keyToValue.ContainsKey(key) || !_valueToKey.ContainsKey(value)) return false;
        _keyToValue.Remove(key);
        _valueToKey.Remove(value);
        return true;
    }
