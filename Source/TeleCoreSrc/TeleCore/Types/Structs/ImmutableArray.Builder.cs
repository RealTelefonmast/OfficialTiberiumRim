...
public partial struct ImmutableArray<T>
{
    ...
    public sealed class Builder : IList<T>, IReadOnlyList<T>
    {
            ...
            set
            {
                ...
                if (value != _elements.Length)
                {
                    if (value > 0)
                    {
                        ...
                        if (_count > 0) System.Array.Copy(_elements, temp, _count);
        ...
        public void Insert(int index, T item)
        {
            ...
            if (index < Count) System.Array.Copy(_elements, index, _elements, index + 1, Count - index);
        ...
        public void RemoveAt(int index)
        {
            ...
            if (index < Count - 1) System.Array.Copy(_elements, index + 1, _elements, index, Count - index - 1);
        ...
        public void CopyTo(T[] array, int index)
        {
            ...
            Requires.Range(index >= 0 && index + Count <= array.Length, nameof(index));
        ...
        ///     When <see cref="ImmutableArray{T}.Builder.Count" /> doesn't
        ///     equal <see cref="ImmutableArray{T}.Builder.Capacity" />.
        /// </exception>
        public ImmutableArray<T> MoveToImmutable()
        {
            if (Capacity != Count) throw new InvalidOperationException("SR.CapacityMustEqualCountOnMove");
        ...
        public void InsertRange(int index, IEnumerable<T> items)
        {
            ...
            if (index != Count) System.Array.Copy(_elements, index, _elements, index + count, _count - index);
        ...
        public void InsertRange(int index, ImmutableArray<T> items)
        {
            ...
            if (index != Count) System.Array.Copy(_elements, index, _elements, index + items.Length, _count - index);
        ...
        public void AddRange(ImmutableArray<T> items, int length)
        {
            ...
            if (items.array != null) AddRange(items.array, length);
        }
        ...
        public void AddRange<TDerived>(ReadOnlySpan<TDerived> items) where TDerived : T
        {
            ...
            for (var i = 0; i < items.Length; i++) elements[i] = items[i];
        }
        ...
        {
            if (items.array != null) AddRange(items.array);
        }
        ...
        {
            var index = IndexOf(element, 0, _count, equalityComparer);
        ...
        public void RemoveAll(Predicate<T> match)
        {
            ...
            if (removeIndices != null) RemoveAtRange(removeIndices);
        }
        ...
        public void RemoveRange(IEnumerable<T> items, IEqualityComparer<T>? equalityComparer)
        {
            ...
            foreach (var item in items)
            {
                var index = IndexOf(item, 0, _count, equalityComparer);
        ...
        {
            var index = IndexOf(oldValue, 0, _count, equalityComparer);

            if (index >= 0) _elements[index] = newValue;
        }
        ...
        public void CopyTo(int sourceIndex, T[] destination, int destinationIndex, int length)
        {
            Requires.NotNull(destination, nameof(destination));
            Requires.Range(length >= 0, nameof(length));
            Requires.Range(sourceIndex >= 0 && sourceIndex + length <= Count, nameof(sourceIndex));
        ...
        public int IndexOf(T item, int startIndex, int count, IEqualityComparer<T>? equalityComparer)
        {
            ...
            Requires.Range(startIndex >= 0 && startIndex < Count, nameof(startIndex));
            Requires.Range(count >= 0 && startIndex + count <= Count, nameof(count));
        ...
        public void Sort()
        {
            if (Count > 1) System.Array.Sort(_elements, 0, Count, Comparer<T>.Default);
        }
        ...
        {
            if (Count > 1) System.Array.Sort(_elements, 0, _count, comparer);
        }
        ...
        public void Sort(int index, int count, IComparer<T>? comparer)
        {
            ...
            Requires.Range(count >= 0 && index + count <= Count, nameof(count));

            if (count > 1) System.Array.Sort(_elements, index, count, comparer);
        }
        ...
        {
            Requires.Range(Count <= destination.Length, nameof(destination));
        ...
        private void AddRange<TDerived>(TDerived[] items, int length) where TDerived : T
        {
            ...
            var nodes = _elements;
            for (var i = 0; i < length; i++) nodes[offset + i] = items[i];
        }
