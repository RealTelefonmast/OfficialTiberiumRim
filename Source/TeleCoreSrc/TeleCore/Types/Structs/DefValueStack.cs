...
{
    [FieldOffset(0)] public int defID; //4 bytes
    [FieldOffset(4)] public Numeric<TValue> _value; //Up to 8 bytes
    ...
    where TValue : unmanaged
{
    ...
    private void TryAddOrSet(UnsafeDefValue<TDef, TValue> newValue)
    {
        if (!TryGetValue(newValue.Def, out var index, out var previous))
        {
            ...
            var nxt = curInd++;
            for (var i = 0; i < 12; i++) _stack[nxt + i] = ptr[i];
        ...
        else
        {
            //Set new value
            var ptr = (byte*)&newValue;
            for (var i = 0; i < 12; i++) _stack[index + i] = ptr[i];
        }
    ...
    where TValue : unmanaged
{
    ...
    public static implicit operator DefValueStack<TDef, TValue>(DefValue<TDef, TValue> value)
    {
        return new DefValueStack<TDef, TValue>(value);
    }
    ...
    public void ExposeData()
    {
        ...
        if (Scribe.mode == LoadSaveMode.LoadingVars)
        {
            ...
            foreach (var val in listTemp) _stack = _stack.Add(val);
        }
    }
    ...
    public DefValueStack(DefValue<TDef, TValue> value) : this()
    {
        ...
        _stack = new LightImmutableArray<DefValue<TDef, TValue>>(
            value); //new ImmutableArray<DefValue<TDef, TValue>>().Add(value);
    ...
    {
        if (a._stack.IsNullOrEmpty) return a;
        if (new Numeric<TValue>(b).IsZero) return new DefValueStack<TDef, TValue>();
        foreach (var value in a.Values)
            a[value.Def] /= b;
        return a;
    }
    ...
    {
        if (a._stack.IsNullOrEmpty) return a;
        if (value.Value.IsZero) return new DefValueStack<TDef, TValue>();
        a[value.Def] /= value;
        return a;
    }
