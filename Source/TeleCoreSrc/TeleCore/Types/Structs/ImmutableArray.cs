    ...
    IImmutableList<T>
{
    ...
    public int IndexOf(T item, int startIndex, int count, IEqualityComparer<T>? equalityComparer)
    {
        ...
        if (equalityComparer == EqualityComparer<T>.Default)
            return System.Array.IndexOf(self.array!, item, startIndex, count);
    ...
    {
        var self = this;
        if (self.IsEmpty && startIndex == 0) return -1;
    ...
    public int LastIndexOf(T item, int startIndex, int count, IEqualityComparer<T>? equalityComparer)
    {
        ...
        equalityComparer ??= EqualityComparer<T>.Default;
        if (equalityComparer == EqualityComparer<T>.Default)
            return System.Array.LastIndexOf(self.array!, item, startIndex, count);
    ...
    public ImmutableArray<T> Insert(int index, T item)
    {
        ...
        if (self.IsEmpty) return ImmutableArray.Create(item);

        var tmp = new T[self.Length + 1];
        tmp[index] = item;

        if (index != 0) System.Array.Copy(self.array!, tmp, index);

        if (index != self.Length) System.Array.Copy(self.array!, index, tmp, index + 1, self.Length - index);
    ...
    public ImmutableArray<T> InsertRange(int index, IEnumerable<T> items)
    {
        ...
        if (self.IsEmpty) return ImmutableArray.CreateRange(items);
        ...
        if (index != self.Length) System.Array.Copy(self.array!, index, tmp, index + count, self.Length - index);
        ...
        if (!items.TryCopyTo(tmp, index))
        {
            ...
            foreach (var item in items) tmp[sequenceIndex++] = item;
        }
    ...
    public ImmutableArray<T> AddRange(T[] items, int length)
    {
        ...
        if (items.Length == 0 || length == 0) return self;

        if (self.IsEmpty) return ImmutableArray.Create(items, 0, length);

        var tmp = new T[self.Length + length];
        System.Array.Copy(self.array!, tmp, self.Length);
        System.Array.Copy(items, 0, tmp, self.Length, length);
    ...
    public ImmutableArray<T> AddRange<TDerived>(TDerived[] items) where TDerived : T
    {
        ...
        System.Array.Copy(self.array!, tmp, self.Length);
        System.Array.Copy(items, 0, tmp, self.Length, items.Length);
    ...
    public ImmutableArray<T> AddRange(ImmutableArray<T> items, int length)
    {
        ...
        if (items.array != null) return self.AddRange(items.array, length);
    ...
    {
        var self = this;
        if (items.array != null) return self.AddRange(items.array);
    ...
    {
        var self = this;
        var index = self.IndexOf(oldValue, 0, self.Length, equalityComparer);
        if (index < 0) throw new ArgumentException("SR.CannotFindOldValue", nameof(oldValue));
    ...
    public ImmutableArray<T> Remove(T item, IEqualityComparer<T>? equalityComparer)
    {
        var self = this;
        self.ThrowNullRefIfNotInitialized();
        var index = self.IndexOf(item, 0, self.Length, equalityComparer);
        return index < 0
            ? self
            : self.RemoveAt(index);
    }
    ...
    public ImmutableArray<T> RemoveRange(int index, int length)
    {
        ...
        System.Array.Copy(self.array!, index + length, tmp, index, self.Length - index - length);
        return new ImmutableArray<T>(tmp);
    }
    ...
    public ImmutableArray<T> Sort(int index, int count, IComparer<T>? comparer)
    {
        ...
        if (count > 1)
        {
            ...
            if (outOfOrder)
            {
                var tmp = new T[self.Length];
                System.Array.Copy(self.array!, tmp, self.Length);
                System.Array.Sort(tmp, index, count, comparer);
                return new ImmutableArray<T>(tmp);
            }
        }
    ...
    {
        var self = this;
        if (self.array == null || self.array.Length == 0) return Enumerable.Empty<TResult>();
    ...
    /// <summary>
    ///     Creates a <see cref="ReadOnlySpan{T}" /> over the portion of current <see cref="ImmutableArray{T}" /> beginning at
    ///     a specified position for a specified length.
    /// </summary>
    ...
    ///     The <see cref="Span{T}" /> that is the destination of the elements copied from current
    ...
    public ImmutableArray<T> InsertRange(int index, T[] items)
    {
        ...
        if (self.IsEmpty) return new ImmutableArray<T>(items);
    ...
    public ImmutableArray<T> InsertRange(int index, ReadOnlySpan<T> items)
    {
        ...
        if (self.IsEmpty) return items.ToImmutableArray();
    ...
    public ImmutableArray<T> RemoveRange(ReadOnlySpan<T> items, IEqualityComparer<T>? equalityComparer = null)
    {
        ...
        if (items.IsEmpty || IsEmpty) return this;

        if (items.Length == 1) return Remove(items[0], equalityComparer);
    ...
    /// <summary>
    ///     Forms a slice out of the current <see cref="ImmutableArray{T}" /> starting at a specified index for a specified
    ///     length.
    /// </summary>
    /// <param name="start">The index at which to begin this slice.</param>
    /// <param name="length">The desired length for the slice.</param>
    /// <returns>
    ///     A <see cref="ImmutableArray{T}" /> that consists of length elements from the current
    ///     <see cref="ImmutableArray{T}" /> starting at start.
    /// </returns>
    ...
    /// <summary>
    ///     Copies the elements of the <see cref="ICollection" /> to an <see cref="System.Array" />, starting at a particular
    ///     <see cref="System.Array" /> index.
    /// </summary>
    /// <param name="array">
    ///     The one-dimensional <see cref="System.Array" /> that is the destination of the elements copied from
    ///     <see cref="ICollection" />. The <see cref="System.Array" /> must have zero-based indexing.
    /// </param>
    ...
    {
        var self = this;
        var otherArray = other as System.Array;
        if (otherArray == null)
            if (other is IImmutableArray theirs)
            {
                otherArray = theirs.Array;

                if (self.array == null && otherArray == null) return true;
    ...
    int IStructuralComparable.CompareTo(object? other, IComparer comparer)
    {
        ...
        if (otherArray == null)
            if (other is IImmutableArray theirs)
            {
                ...
                if (self.array == null && otherArray == null) return 0;
        ...
        if (otherArray != null)
        {
            ...
            if (ours == null) throw new ArgumentException("SR.ArrayInitializedStateNotEqual", nameof(other));
    ...
    private ImmutableArray<T> RemoveAtRange(ICollection<int> indicesToRemove)
    {
        ...
        foreach (var indexToRemove in indicesToRemove)
        {
            ...
            System.Array.Copy(self.array!, copied + removed, newArray, copied, copyLength);
            removed++;
            copied += copyLength;
            lastIndexRemoved = indexToRemove;
        }

        System.Array.Copy(self.array!, copied + removed, newArray, copied, self.Length - (copied + removed));
    ...
    private ImmutableArray<T> InsertSpanRangeInternal(int index, ReadOnlySpan<T> items)
    {
        ...
        if (index != 0) System.Array.Copy(array!, tmp, index);

        items.CopyTo(new Span<T>(tmp, index, items.Length));
        if (index != Length) System.Array.Copy(array!, index, tmp, index + items.Length, Length - index);
