    ...
    private sealed class ImmutableArrayEnumeratorObject : IEnumerator<T>
    {
        ...
        public T Current
        {
            get
            {
                ...
                if (unchecked((uint)_index) < (uint)_array.Length) return _array[_index];
        ...
        {
            if (array.Length != 0) return new ImmutableArrayEnumeratorObject(array);
...
public partial struct LightImmutableArray<T> : IImmutableArray<T>, ICollection<T>, IEnumerable<T>,
    IStructuralComparable, IStructuralEquatable
{
    ...
    public T this[int i]
    {
        ...
        set => TLog.Error($"Cannot set ImmutableArray directly, use {nameof(SetAt)} instead!");
    }

    public static readonly LightImmutableArray<T> Empty = new(System.Array.Empty<T>());
    ...
    {
        var self = this;
        if (self.IsEmpty) return new LightImmutableArray<T>(item);
    ...
    public int IndexOf(T item, int startIndex, int count, IEqualityComparer<T>? equalityComparer)
    {
        ...
        if (Equals(equalityComparer, EqualityComparer<T>.Default))
            return System.Array.IndexOf(Array, item, startIndex, count);
    ...
    public static bool operator !=(LightImmutableArray<T> arr, System.Array? obj)
    {
    ...
    public int CompareTo(object other, IComparer comparer)
    {
        ...
        if (otherArray == null)
            if (other is IImmutableArray<T> theirs)
            {
                ...
                if (Array == null && otherArray == null) return 0;
