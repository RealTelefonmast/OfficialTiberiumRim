...
public record struct IOConnection
{
    ...
    public bool IsValid => FromPos.Pos.IsValid && ToPos.Pos.IsValid && FromMode != NetworkIOMode.None &&
    ...
    public IOConnection Reverse => new()
    {
        From = To,
        To = From,
        FromMode = ToMode,
    ...
    {
        if (part == From) return (FromMode, FromPos);
        if (part == To) return (ToMode, ToPos);
