...
public sealed class ModLogger : ILogger
{
    ...
    public void ErrorOnce(string msg, int id)
    {
        Log.ErrorOnce($"{Tag} {msg}", id);
    }
    ...
    {
        Log.Message($"{_tag.Colorize(color)} {msg}");
    }
    ...
    public void DebugSuccess(string msg)
    {
        Log.Message($"{DebugTag} {msg}");
    }
    ...
    public void DebugOnce(string msg, int hash)
    {
        ...
        lock (lockObj)
        {
            if (!Log.reachedMaxMessagesLimit && Log.usedKeys.Add(hash)) Debug(msg);
        }
    }
}