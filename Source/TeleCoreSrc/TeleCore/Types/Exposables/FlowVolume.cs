...
public abstract class FlowVolumeBase<TValue> : IExposable, INotifyFlowEvent where TValue : FlowValueDef
{
    ...
    public float FillPercent => TotalValue / MaxCapacity;
    ...
    {
        return Mathf.Max(TotalValue + amount - CapacityPerType, 0);
    }
    ...
    public void RegenColorState()
    {
        ...
        foreach (var value in _mainStack) newColor += value.Value.Value / MaxCapacity * value.Def.valueColor;
    ...
    private void OnContainerStateChanged(float delta, bool updateMetaData = false)
    {
        ...
        //Resolve Action
        var action = VolumeChangedEventArgs<TValue>.ChangedAction.Invalid;
        if (delta > 0)
    ...
    private Color SaturateColor(Color color)
    {
        ...
        if (avg > max) return new Color(color.r / max, color.g / max, color.b / max);
    ...
    public FlowResultStack<TValue> TryTake(DefValueStack<TValue, float> stack)
    {
        ...
        foreach (var value in stack)
        {
            ...
            if (subResult) result.AddResult(subResult);
        }
    ...
    {
        Clear();
        foreach (var defVal in stack) _ = TryAdd(defVal.Def, defVal.Value);
    }
    ...
    private bool ValueOperationCheck(FlowOperation operation, DefValue<TValue, float> value,
        out FlowFailureReason reason)
    {
    ...
    private static bool CanTransferTo(FlowVolumeBase<TValue> other, TValue def, float value,
        out FlowFailureReason flowFailureReason)
    {
        ...
        if (other.AllowedValues.Contains(def)) return true;
    ...
    /*public FlowResult<T, float> TryRemove(ICollection<DefValue<T, float>> values)
    {
    var result = new FlowResult<T, float>();
    ...
    /// </summary>
    public FlowResult<TValue, float>
        TryTransferOrFail(FlowVolumeBase<TValue> other, DefValue<TValue, float> value) //ALL OR NOTHING
    {
        if (CanTransferTo(other, value.Def, value.Value, out FlowFailureReason reason))
        {
            //return TryTransfer(other, value);
            var removeResult = TryRemove(value.Def, value.Value);
            if (removeResult) return other.TryAdd(removeResult.Def, removeResult.Actual);
        }
    ...
    /// </summary>
    public FlowResult<TValue, float>
        TryTransfer(FlowVolumeBase<TValue> other, DefValue<TValue, float> value) //AS MUCH AS POSSIBLE
    {
        var remResult = TryRemove(value.Def, value.Value);
        if (remResult) return other.TryAdd(remResult.Def, remResult.Actual);
        return remResult;
    }
    ...
    public FlowResult<TValue, float> TryConsumeOrFail(TValue def, float amount) //ALL OR NOTHING
    {
        ...
        return FlowResult<TValue, float>.InitFailed(def, amount,
            FlowFailureReason.TriedToConsumeMoreThanExists); //value.Value
    }
...
public class FlowVolumeShared<T> : FlowVolumeBase<T> where T : FlowValueDef
{
    ...
    public virtual float MaxCapacity => _config.capacity * AllowedValues.Count;
