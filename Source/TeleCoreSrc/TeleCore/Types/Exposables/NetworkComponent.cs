...
public class NetworkComponent : IExposable, INetworkComponent, IContainerHolderStructure
{
    ...
    //DEBUG
    protected bool DebugNetworkCells;

    private bool drawNetworkInfo;
    ...
    public Dictionary<NetworkValueDef, (bool, float)> RequestedTypes { get; private set; }
    ...
    public RequesterMode RequesterMode { get; set; } = RequesterMode.Automatic;
    ...
    {
        if (NetworkRole.HasFlag(this.NetworkRole.Requester))
        {
            RequestedTypes = new Dictionary<NetworkValueDef, (bool, float)>();
            foreach (var allowedValue in Props.AllowedValuesByRole[this.NetworkRole.Requester])
    ...
    private void ProcessValues()
    {
        ...
        if (NetworkRole.HasFlag(this.NetworkRole.Producer)) ProducerTick();

        //Storages push to Consumers
        if (NetworkRole.HasFlag(this.NetworkRole.Storage)) StorageTick();

        //Consumers slowly use up own container
        if (NetworkRole.HasFlag(this.NetworkRole.Consumer)) ConsumerTick();

        if (NetworkRole.HasFlag(this.NetworkRole.Requester)) RequesterTick();
    }

    protected virtual void ProducerTick()
    {
        TransferToOthers(this.NetworkRole.Producer, this.NetworkRole.Storage, false);
    }

    protected virtual void StorageTick()
    {
        if (Container.ContainsForbiddenType) PushForbiddenTypes();
        if (ContainerProps.storeEvenly) TransferToOthers(this.NetworkRole.Storage, this.NetworkRole.Storage, true);
        TransferToOthers(this.NetworkRole.Storage, this.NetworkRole.Consumer, false);
    }
    ...
    protected virtual void RequesterTick()
    {
        if (RequesterMode == RequesterMode.Automatic)
        {
            ...
            foreach (var valType in Props.AllowedValuesByRole[this.NetworkRole.Requester])
            {
                var valTypeValue = Container.ValueForType(valType) +
                                   Network.NetworkValueFor(valType, this.NetworkRole.Storage);
                if (valTypeValue > 0)
                {
        ...
        foreach (var requestedType in RequestedTypes)
        {
            ...
            if (Container.ValueForType(requestedType.Key) < requestedType.Value.Item2)
                foreach (var component in Network.ComponentSet[this.NetworkRole.Storage])
                {
                    var container = component.Container;
                    if (container.Empty) continue;
                    if (container.ValueForType(requestedType.Key) <= 0) continue;
                    if (container.TryTransferTo(Container, requestedType.Key, 1)) Notify_ReceivedValue();
                }
        }
    }
    ...
    private void TransferToOthers(NetworkRole fromRole, NetworkRole ofRole, bool evenly)
    {
        ...
        foreach (var component in Network.ComponentSet[ofRole])
        {
            ...
            for (var i = usedTypes.Count - 1; i >= 0; i--)
            {
                ...
                if (Container.TryTransferTo(component.Container, type, 1)) component.Notify_ReceivedValue();
            }
        }
    }
    ...
    private void PushForbiddenTypes()
    {
        ...
        foreach (var component in Network.ComponentSet[this.NetworkRole.Storage])
        {
            ...
            for (var i = Container.AllStoredTypes.Count - 1; i >= 0; i--)
            {
                var type = Container.AllStoredTypes.ElementAt(i);
                if (Container.AcceptsType(type)) continue;
                if (!component.NeedsValue(type, this.NetworkRole.Storage)) continue;
                if (Container.TryTransferTo(component.Container, type, 1)) component.Notify_ReceivedValue();
            }
        }
    }
    ...
    {
        DrawNetworkInfo();
        if (DebugNetworkCells) GenDraw.DrawFieldEdges(Network.NetworkCells, Color.cyan);
    }
        ...
        if (DebugSettings.godMode)
        {
            if (IsMainController)
                yield return new Command_Action
                {
                    ...
                    action = delegate { DebugNetworkCells = !DebugNetworkCells; }
                };
