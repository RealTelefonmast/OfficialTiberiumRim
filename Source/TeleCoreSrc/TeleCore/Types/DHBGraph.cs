...
public unsafe class DHBGraph
{
    ...
    private readonly DHBAllocator allocator = new();
    ...
    {
        if (!vertices.ContainsKey(id)) vertices[id] = new Vertex(INITIAL_BLOCK_SIZE);
    }

    public void AddEdge(int source, int target)
    {
        AddVertex(source);
        var vertex = vertices[source];

        if (vertex.Degree >= vertex.BlockSize) ReallocateBlock(vertex);

        if (vertex.Degree >= HIGH_DEGREE_THRESHOLD && vertex.HashIndex == null) InitializeHashIndex(vertex);
    ...
    {
        var newSize = vertex.BlockSize == 0 ? 1 : vertex.BlockSize * 2;
        var newNeighbors = (int*)allocator.Allocate(newSize * sizeof(int));
    ...
    private void InitializeHashIndex(Vertex vertex)
    {
        ...
        for (var i = 0; i < vertex.BlockSize; i++)
    ...
    {
        var newHashIndex = (int*)Marshal.AllocHGlobal(newSize * sizeof(int));
    ...
    {
        var hash = Hash(target) & (vertex.BlockSize - 1);
        while (true)
        {
            var index = vertex.HashIndex[hash];
