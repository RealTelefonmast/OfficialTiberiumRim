...
{
    private readonly NetworkDef def;
    private readonly INetworkComponent parent;
    ...
    private void AddComponent(INetworkComponent component)
    {
        ...
        if (component.NetworkRole.HasFlag(NetworkRole.Controller)) Controller = component;
        if (component.NetworkRole.HasFlag(NetworkRole.Transmitter)) Transmitters.Add(component);
        if (component.NetworkRole.HasFlag(NetworkRole.Producer)) Producers.Add(component);
        if (component.NetworkRole.HasFlag(NetworkRole.Consumer)) Consumers.Add(component);
        if (component.NetworkRole.HasFlag(NetworkRole.Storage)) Storages.Add(component);
        UpdateString(0);
    }
    ...
    {
        if (!FullSet.Contains(component)) return;
        if (component.NetworkRole.HasFlag(NetworkRole.Controller)) Controller = null;
        if (component.NetworkRole.HasFlag(NetworkRole.Transmitter)) Transmitters.Remove(component);
        if (component.NetworkRole.HasFlag(NetworkRole.Producer)) Producers.Remove(component);
        if (component.NetworkRole.HasFlag(NetworkRole.Consumer)) Consumers.Remove(component);
        if (component.NetworkRole.HasFlag(NetworkRole.Storage)) Storages.Remove(component);
    ...
    {
        foreach (var ns in FullSet) ns.Notify_NewComponentRemoved(parent);
    }
    ...
    public void UpdateString(int index)
    {
        ...
        if (index == 0)
        {
            ...
            foreach (var ns in Transmitters) sb.AppendLine($"    - {ns.Parent.Thing}");
            sb.AppendLine("Producers: ");
            foreach (var ns in Producers) sb.AppendLine($"    - {ns.Parent.Thing}");
            sb.AppendLine("Consumers: ");
            foreach (var ns in Consumers) sb.AppendLine($"    - {ns.Parent.Thing}");
            sb.AppendLine("Storages: ");
            foreach (var ns in Storages) sb.AppendLine($"    - {ns.Parent.Thing}");
