...
public static class ImmutableArray
{
    ...
    {
        if (items.IsEmpty) return ImmutableArray<T>.Empty;
    ...
    public static ImmutableArray<T> CreateRange<T>(IEnumerable<T> items)
    {
        ...
        {
            var array = immutableArray.Array;
            if (array == null) throw new InvalidOperationException("SR.InvalidOperationOnDefaultArray");
    ...
    {
        if (items == null || items.Length == 0) return ImmutableArray<T>.Empty;
    ...
    public static ImmutableArray<T> Create<T>(T[] items, int start, int length)
    {
        ...
        for (var i = 0; i < array.Length; i++) array[i] = items[start + i];
    ...
    public static ImmutableArray<T> Create<T>(ImmutableArray<T> items, int start, int length)
    {
        ...
        if (length == 0) return Create<T>();

        if (start == 0 && length == items.Length) return items;

        var array = new T[length];
        System.Array.Copy(items.array!, start, array, 0, length);
        return new ImmutableArray<T>(array);
    }
        ...
        Func<TSource, TResult> selector)
    {
        ...
        if (length == 0) return Create<TResult>();

        var array = new TResult[length];
        for (var i = 0; i < array.Length; i++) array[i] = selector(items[i]);
    ...
    ///     <see cref="ImmutableArray{T}" />, where a mapping function needs to be applied to each element from the source
    ///     array
        ...
        int length, Func<TSource, TResult> selector)
    {
        ...
        var array = new TResult[length];
        for (var i = 0; i < array.Length; i++) array[i] = selector(items[i + start]);
        ...
        Func<TSource, TArg, TResult> selector, TArg arg)
    {
        ...
        var array = new TResult[length];
        for (var i = 0; i < array.Length; i++) array[i] = selector(items[i], arg);
        ...
        int length, Func<TSource, TArg, TResult> selector, TArg arg)
    {
        ...
        var array = new TResult[length];
        for (var i = 0; i < array.Length; i++) array[i] = selector(items[i + start], arg);
    ...
    {
        if (items is ImmutableArray<TSource>) return (ImmutableArray<TSource>)items;
    ...
    ///     element that is larger than <paramref name="value" />. If <paramref name="value" /> is not found and
    ...
    {
        return System.Array.BinarySearch<T>(array.array!, value);
    }
    ...
    ///     <see cref="IComparable{T}" /> generic interface, and
    ///     the search encounters an element that does not implement the <see cref="IComparable{T}" />
    ///     generic interface.
    /// </exception>
    public static int BinarySearch<T>(this ImmutableArray<T> array, T value, IComparer<T>? comparer)
    {
        return System.Array.BinarySearch<T>(array.array!, value, comparer);
    }
    ...
    {
        return System.Array.BinarySearch<T>(array.array!, index, length, value);
    }
    ...
    {
        return System.Array.BinarySearch<T>(array.array!, index, length, value, comparer);
    }
}