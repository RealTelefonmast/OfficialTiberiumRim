...
internal static partial class ImmutableExtensions
{
    ...
    internal static IOrderedCollection<T> AsOrderedCollection<T>(this IEnumerable<T> sequence)
    {
        ...
        if (sequence is IOrderedCollection<T> orderedCollection) return orderedCollection;

        if (sequence is IList<T> listOfT) return new ListOfTWrapper<T>(listOfT);
    ...
    ///     avoids a call into the runtime's implementation of <see cref="System.Array.Clear" />, helping performance,
    ///     in particular around inlining.  <see cref="Stack{T}.Count" /> typically gets inlined by today's JIT, while
    ///     <see cref="Stack{T}.Clear" /> and <see cref="System.Array.Clear" /> typically don't.
    /// </summary>
    ...
    private sealed class FallbackWrapper<T> : IOrderedCollection<T>
    {
        ...
        public int Count
        {
            get
            {
                ...
                {
                    int count;
                    if (_sequence.TryGetCount(out count)) return count;
