...
internal static partial class ImmutableExtensions
{
    ...
    ///         implementation of <see cref="ICollection{T}" /> could hold on to the array when its
    ...
    internal static bool TryCopyTo<T>(this IEnumerable<T> sequence, T[] array, int arrayIndex)
    {
        ...
        if (sequence is IList<T>)
        {
            ...
            if (sequence.GetType() == typeof(T[]))
            {
                ...
                System.Array.Copy(sourceArray, 0, array, arrayIndex, sourceArray.Length);
                return true;
            }

            if (sequence is ImmutableArray<T> immutable)
            {
                System.Array.Copy(immutable.array!, 0, array, arrayIndex, immutable.Length);
                return true;
            }
        }
    ...
    /// <remarks>
    ///     This is more efficient than the <see cref="System.Linq.Enumerable.ToArray{TSource}(IEnumerable{TSource})" />
    ///     extension method
    ...
    internal static T[] ToArray<T>(this IEnumerable<T> sequence, int count)
    {
        ...
        if (count == 0) return ImmutableArray<T>.Empty.array!;
