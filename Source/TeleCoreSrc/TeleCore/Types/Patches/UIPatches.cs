    ...
    */
    ...
    public static class PlaySettingsPatch
    {
        public static void Postfix(WidgetRow row, bool worldView)
        {
            ...
            SelectAtmosphere(row, AtmosphericContent.AtmosphereIcon, "TAE_ToggleAtmosphereLayer".Translate(),
                SoundDefOf.Mouseover_ButtonToggle);
            row.ToggleableIcon(ref AtmosphereMod.Mod.Settings.DrawAtmosphereAroundMouse, BaseContent.BadTex,
                "Toggle AtmosLayer");
        }

        private static void SelectAtmosphere(WidgetRow row, Texture2D tex, string tooltip,
            SoundDef mouseoverSound = null)
        {
            row.IncrementYIfWillExceedMaxWidth(24f);
            var rect = new Rect(row.LeftX(24f), row.curY, 24f, 24f);
            var flag = Widgets.ButtonImage(rect, tex);
            row.IncrementPosition(24f + row.gap);
            if (!tooltip.NullOrEmpty()) TooltipHandler.TipRegion(rect, tooltip);
            var position = new Rect(rect.x + rect.width / 2f, rect.y, rect.height / 2f, rect.height / 2f);
            var image = AtmosphereMod.Mod.Settings.DrawAtmospheres ? Widgets.CheckboxOnTex : Widgets.CheckboxOffTex;
            GUI.DrawTexture(position, image);

            if (mouseoverSound != null) MouseoverSounds.DoRegion(rect, mouseoverSound);

            if (flag)
                Find.CurrentMap.GetMapInfo<AtmosphericMapInfo>().Renderer
                    .OpenAtmosphereLayerMenu(AtmosphereSelectionCallback);
        }
    ...
    private static class MouseoverReadout_Patch
    {
        private static readonly MethodBase ReadoutCall =
            AccessTools.Method(typeof(MouseoverReadout_Patch), nameof(DrawGasReadout));
        ...
        private static void DrawGasReadout(ref float curYOffset)
        {
            ...
            foreach (var setting in StaticData.PlaySettings)
                if (setting.Visible)
                    if ((worldView && setting.ShowOnWorldView) || (!worldView && setting.ShowOnMapView))
                        if (row.ButtonIcon(setting.ActiveIcon, setting.Description))
                            setting.Toggle();
        }
    }
