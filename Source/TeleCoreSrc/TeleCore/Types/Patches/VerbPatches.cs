...
internal static class VerbPatches
{
    ...
    internal static class VerbLaunchProjectile_TryCastShot_Patch
    {
        ...
        private static readonly MethodInfo TransformCastOriginMethod =
            AccessTools.Method(typeof(VerbLaunchProjectile_TryCastShot_Patch), nameof(TransformCastOrigin));

        private static readonly FieldInfo CachedProjectileFld =
            AccessTools.Field(typeof(VerbLaunchProjectile_TryCastShot_Patch), nameof(_projectile));
        ...
        internal static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            foreach (var instruction in instructions)
            {
                ...
                {
                    foreach (var subInstruction in sub1) yield return subInstruction;
                    continue;
                }

                if (TryInjectCacheLaunchedProjectile(instruction, out var sub2))
                {
                    foreach (var subInstruction in sub2) yield return subInstruction;
                    continue;
                }
        ...
        private static bool TryInjectProjectileLaunchPosition(CodeInstruction instruction,
            out IEnumerable<CodeInstruction> instructions)
        {
        ...
        private static bool TryInjectCacheLaunchedProjectile(CodeInstruction instruction,
            out IEnumerable<CodeInstruction> instructions)
        {
            instructions = Enumerable.Empty<CodeInstruction>();
