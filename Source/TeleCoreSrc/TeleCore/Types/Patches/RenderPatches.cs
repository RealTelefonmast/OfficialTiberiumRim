...
internal static class RenderPatches
{
    ...
    */
    ...
    [HarmonyPatch(new[] { typeof(BuildableDef) })]
    ...
    {
        internal static readonly Dictionary<ThingDef, int> _indexes = new();
        ...
        {
            if (!_indexes.TryAdd(def, index)) _indexes[def] = index;
        }
    }
...
//TODO: Move to FlowCore
    ...
    public static class GhostUtilityGhostGraphicForPatch
    {
        ...
        private static Graphic GhostGraphicCopy(Graphic? baseGraphic, ThingDef thingDef, Color ghostCol,
            ThingDef stuff = null)
        {
            var num = 0;
            ...
            if (!GhostUtility.ghostGraphics.TryGetValue(num, out var graphic))
            {
                ...
                {
                    graphic = GraphicDatabase.Get<Graphic_Single>(thingDef.uiIconPath,
                        ShaderTypeDefOf.EdgeDetect.Shader, thingDef.graphicData.drawSize, ghostCol);
                }
                else
                {
                    if (baseGraphic == null) baseGraphic = thingDef.graphic;
                    ...
                    var path = baseGraphic.path;
                    Graphic_Appearances graphicAppearances;
                    if ((graphicAppearances = baseGraphic as Graphic_Appearances) != null && stuff != null)
                        graphic = GraphicDatabase.Get<Graphic_Single>(graphicAppearances.SubGraphicFor(stuff).path,
                            ShaderTypeDefOf.EdgeDetect.Shader, thingDef.graphicData.drawSize, ghostCol, Color.white,
                            graphicData);
                    else
                        graphic = GraphicDatabase.Get(baseGraphic.GetType(), path, ShaderTypeDefOf.EdgeDetect.Shader,
                            baseGraphic.drawSize, ghostCol, Color.white, graphicData, null);
                }
