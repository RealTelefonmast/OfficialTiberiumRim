...
public class IOTests
{
    ...
    public void ConnectionTest()
    {
        ...
        var io1 = new NetworkIO(config, new IntVec3(3, 0, 3), Rot4.North);
        var io2 = new NetworkIO(config, new IntVec3(4, 0, 3), Rot4.North);
    ...
    public static void DrawCells(List<IOCellPrototype> cells, IntVec2 size)
    {
        ...
        for (var y = 0; y < size.z + 2; y++)
            text += "#";

        var center = new IntVec3(Math.Abs(0 - (size.x + 2 - 1) / 2), 0, Math.Abs(0 - (size.z + 2 - 1) / 2));
