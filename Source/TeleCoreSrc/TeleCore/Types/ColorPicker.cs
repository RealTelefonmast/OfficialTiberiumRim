...
public class ColorPicker
{
    ...
    private const float mid_divide = 10;
    ...
    private readonly Material blueMat;
    private readonly Material colorPickerMat;
    private readonly Material greenMat;
    private readonly Material hueMat;

    private readonly Material redMat;
    private readonly Material satMat;
    private readonly Material valMat;
    ...
    static ColorPicker()
    {
        DefaultSize = GetSize(out _, out _);
    }
    ...
    public ColorPicker()
    {
        ...
        sliderThumbStyle.normal.background = sliderThumbStyle.active.background =
            sliderThumbStyle.focused.background = sliderThumbStyle.hover.background = sliderTex;
    ...
    public Rect Draw(Vector2 pos, float extraWidth = 0)
    {
        ...
        //
        var div = new RectDivider(rightSide, GetHashCode(), new Vector2(5, 5));
        ...
        var uvPos = new Vector2(_HSVRaw.y, 1 - _HSVRaw.z);
        ...
        var rSelChanged =
            TWidgets.Image.DrawTextureWithSlider(RGB_R, new UIMaterial(BaseContent.BadTex, redMat), ref redF, 0, 255);
        var gSelChanged =
            TWidgets.Image.DrawTextureWithSlider(RGB_G, new UIMaterial(BaseContent.BadTex, greenMat), ref greenF, 0,
                255);
        var bSelChanged =
            TWidgets.Image.DrawTextureWithSlider(RGB_B, new UIMaterial(BaseContent.BadTex, blueMat), ref blueF, 0, 255);
        ...
        var hexChanged = TWidgets.Text.TextFieldHex(RGB_HEX_Input, out var hexValue, hexString, ContextSpace);
        ...
        if (rChanged || gChanged || bChanged) RGBChanged(new ColorInt(red, green, blue));
        ...
        var hSelChanged =
            TWidgets.Image.DrawTextureWithSlider(HSV_H, new UIMaterial(BaseContent.BadTex, hueMat), ref hF, 0, 360);
        var sSelChanged =
            TWidgets.Image.DrawTextureWithSlider(HSV_S, new UIMaterial(BaseContent.BadTex, satMat), ref sF, 0, 100);
        var vSelChanged =
            TWidgets.Image.DrawTextureWithSlider(HSV_V, new UIMaterial(BaseContent.BadTex, valMat), ref vF, 0, 100);
        ...
        if (hChanged || sChanged || vChanged) HSVChanged(hInt, sInt, vInt);
    ...
    private void OnSelectingInPicker(Rect inRect, Vector2 uvPos, Color color)
    {
        ...
        switch (type)
        {
            ...
            case EventType.MouseDrag:
            {
                if (GUIUtility.hotControl == id)
                {
                    var newPos = new Vector2(Event.current.mousePosition.x - inRect.x,
                        Event.current.mousePosition.y - inRect.y);
