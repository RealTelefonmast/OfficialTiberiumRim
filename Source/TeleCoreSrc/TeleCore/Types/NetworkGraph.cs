...
public class NetworkGraph : IDisposable
{
    ...
    public Dictionary<(NetworkPart, IOCell), NetEdge> EdgesByIO { get; }
    ...
    private void ValidateAdded(NetEdge edge)
    {
        ...
        if (!AdjacencyList.ContainsKey(edge.From) || !AdjacencyList.ContainsKey(edge.To))
            throw new InvalidOperationException(
                "Nodes of edge have not been added correctly to the AdjacencyList. (Missing Keys)");

        if (!AdjacencyList[edge.From].Exists(e => e.Item2.Value == edge.To) ||
            !AdjacencyList[edge.To].Exists(e => e.Item2.Value == edge.From))
            throw new InvalidOperationException(
                "Nodes of edge have not been added correctly to the AdjacencyList. (Missing Values)");
        ...
        if (!EdgesByIO.TryGetValue((edge.From, edge.FromIOCell), out var edgeFromIOCell) ||
            !edgeFromIOCell.Equals(edge))
    ...
    public bool TryDissolveNode(NetworkPart node)
    {
        ...
        {
            foreach (var (edge, _) in list) DeregisterEdge(edge);
    ...
    ///     Warning: The edges are not guaranteed to have the same order as the request. ie: From being the part having
    ///     adjacency searched for.
    /// </summary>
    ...
    internal void OnGUI()
    {
        if (TeleCoreDebugViewSettings.DrawGraphOnGUI) DebugTools.Debug_DrawGraphOnUI(this);
    }
}