...
{
    private readonly BoolGrid _ioConnGrid;
    private readonly BoolGrid _netGrid;
    ...
    {
        _map = map;
        TotalPartSet = new NetworkPartSet(def);
    ...
    public NetworkPartSet TotalPartSet { get; }

    public void Notify_PartBecameJunction(NetworkPart part)
    {
        TLog.Debug(
            $"Part became junction: {part} | Edge: {part.IsEdge} | Junction: {part.IsJunction} | Node: {part.IsNode}");
    }

    public void Notify_PartSpawned(NetworkPart part)
    {
        //Basic data setup for each spawned part
        if (part.IsNode) FlowSystem.Notify_NewNode(part);

        part.Network = _network;
        Graph.AddCells(part);

        foreach (var cell in part.Thing.OccupiedRect()) _netGrid.Set(cell, true);

        foreach (var visualCell in part.PartIO.VisualCells) _ioConnGrid.Set(visualCell, true);

        //Graph Gen
        var isEdge = part.IsEdge; //When node, search all connections to other nodes
        var isNode =
            part.IsNode; //When edge, seek in two directions to find connected nodes, an edge can ALWAYS only find two connecting nodes
        var isJunction =
            part.IsJunction; //Junctions are a special case that act like nodes when spawned as one (spawned inbetween 3 different edges)
        ...
        {
            if (part.AdjacentSet.Size > 2) TLog.Warning("A pure edge cannot have more than 2 connections!");
            ...
            if (!foundEdge)
            {
                ...
                if (anyEdge != null)
                {
                    ...
                    if (node != null)
                    {
                        ...
                        if (edge.IsValid) newEdges.Add(edge);
                    }
                }
            }
        }
    ...
    private static IEnumerable<NetEdge> GetAllEdgesFor(NetworkPart rootNode)
    {
        ...
        foreach (var conn in rootNode.AdjacentSet.Connections)
        {
            ...
            if (directPart.IsNode)
            {
                ...
                if (directPart.IsJunction) TLog.Warning("Connected to direct junction");

                //TODO: Anomalies with mono-directional connections (ie. Output to Input)
                var connResult = rootNode.IOConnectionTo(directPart);
                if (connResult) yield return new NetEdge(connResult, connResult, 0);
            }
            //Edge traversal - [S|=|=|N]
    ...
    private static (INetworkPart lastEdge, INetworkPart node) FindNextNode(NetworkPart origin, INetworkPart direction)
    {
        ...
        do
        {
            foreach (var item in openSet) closedSet.Add(item);
            ...
            foreach (var connPair in part.AdjacentSet.Connections)
            {
                ...
                if (newPart.IsNode) return (part, newPart);
                openSet.Add(newPart);
            }
    ...
    private static INetworkPart FindNextNode(INetworkPart rootPart)
    {
        ...
        do
        {
            ...
            foreach (var connPair in part.AdjacentSet.Connections)
            {
                ...
                if (newPart.IsNode) return newPart;
                openSet.Add(newPart);
            }
        } while (openSet.Count > 0);
    ...
    private static NetEdge FindNextNode(INetworkPart edgePart, INetworkPart firstNode, IOConnection rootIO)
    {
        ...
        do
        {
            ...
            foreach (var connPair in curEdgePart.AdjacentSet.Connections)
            {
                ...
                if (newPart.IsNode) return new NetEdge(rootIO, io, curLength);
    ...
    public void Tick(bool shouldTick, int tick)
    {
        ...
        if (shouldTick) _network.Tick(tick);
    }
    ...
    public void Draw()
    {
        ...
        foreach (var cell in _ioConnGrid.ActiveCells) CellRenderer.RenderCell(cell, _network.GetHashCode() / 10000f);
    }
