...
namespace TeleCore.Unsorted
{
    public class TurretGun : IAttackTargetSearcher
    {
        ...
        public int BurstCoolDownTicksLeft { get; private set; }

        public bool HoldFire { get; }

        public LocalTargetInfo ForcedTarget => _forcedTarget;

        //Local Turret Data
        public TurretProperties Props { get; private set; }

        public Thing Gun => _gun;
        public TurretGunTop Top { get; private set; }
        ...
        public bool CanSetForcedTarget =>
            (Parent.MannableComp != null || Props.canForceTarget) && Parent.PlayerControlled;
                    ...
                    else
                    {
                        if (BurstCoolDownTicksLeft > 0)
                        {
                            ...
                            if (IsMortar)
                            {
                                ...
                                mote.progress = 1f - TMath.Max(BurstCoolDownTicksLeft, 0) /
        ...
        private void TryStartShootSomething(bool canBeginBurstImmediately)
        {
            ...
            if (!ParentThing.Spawned || (HoldFire && CanToggleHoldFire) ||
                (NeedsRoofless && !ParentThing.Map.roofGrid.Roofed(ParentThing.Position)) || !AttackVerb.Available())
            {
                ResetCurrentTarget();
                return;
            }
            ...
            if (CurrentTarget.IsValid)
            {
                ...
                if (canBeginBurstImmediately)
                    ...
                    _burstWarmupTicksLeft = Props.turretBurstWarmupTime.SecondsToTicks();
            }
            else
            {
                ResetCurrentTarget();
        ...
        public void DrawSelectionOverlays(Vector3 drawLoc)
        {
            ...
            if (WarmingUp)
            {
                ...
                var rot = Top?.CurRotation ?? (CurrentTarget.Thing.DrawPos - DrawPos).AngleFlat();
        ...
        private LocalTargetInfo TryFindNewTarget()
        {
            ...
            if (Rand.Value < 0.5f && AttackVerb.ProjectileFliesOverhead() && faction.HostileTo(Faction.OfPlayer) &&
                ParentThing.Map.listerBuildings
            ...
            if (NeedsRoofless) targetScanFlags |= TargetScanFlags.NeedNotUnderThickRoof;
        ...
        {
            if (Parent.MannableComp is { MannedNow: true }) return Parent.MannableComp.ManningPawn;
            return this;
        }
        ...
        public void TryOrderAttack(LocalTargetInfo targ)
        {
            ...
            //Out of Range
            if ((targ.Cell - ParentThing.Position).LengthHorizontal <
                AttackVerb.verbProps.EffectiveMinRange(targ, Parent.Caster))
            {
                Messages.Message("MessageTargetBelowMinimumRange".Translate(), null, MessageTypeDefOf.RejectInput,
                    false);
                return;
            }
            ...
            {
                Messages.Message("MessageTargetBeyondMaximumRange".Translate(), null, MessageTypeDefOf.RejectInput,
                    false);
                return;
            }
