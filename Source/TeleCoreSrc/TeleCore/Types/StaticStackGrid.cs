    ...
    where TValue : unmanaged
{
    ...
    public int DefID { get; } = 0;

    public TDef Def => DefID<TDef>.ToDef(DefID);
    ...
    public static implicit operator TValue(StaticValue<TDef, TValue> val)
    {
        return val.Value;
    }
    ...
    public override int GetHashCode()
    {
        ...
        {
            var hash = 17;
            hash = hash * 23 + DefID.GetHashCode();
    ...
    public override bool Equals(object obj)
    {
        ...
        return DefID == other.DefID && _value == other._value && _overflow == other._overflow;
    }

    public override string ToString()
    {
        return $"[{DefID<TDef>.ToDef(DefID)}]: ({_value}, {_overflow})";
    }
}
...
public unsafe class StaticStackGrid<TDef, TValue>
    ...
    where TDef : Def
{
    private readonly TDef[] _defArray;
    private readonly int _defCount;
    ...
    public StaticStackGrid(Map map)
    {
        ...
        _grid = new NativeArray<StaticStack<TDef, TValue>>(_gridSize,
            Allocator.Persistent); // new GasCellStack[gridSize];
        ...
        for (var c = 0; c < _gridSize; c++) _gridPtr[c] = new StaticStack<TDef, TValue>();
    }
    ...
    {
        for (var i = 0; i < _defCount; i++) SetCellValueAt(index, value[i]);
    }

    private void AdjustValue(ref StaticValue<TDef, TValue> cellValue, TDef def, Numeric<TValue> value,
        out Numeric<TValue> actualValue)
    {
