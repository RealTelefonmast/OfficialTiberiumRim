...
namespace TeleCore.Rendering.Widgets;
...
public static class WidgetStackPanel
{
    ...
    public static void DrawWidgetRow(Verse.WidgetRow row)
    {
    ...
    public static void DrawRow(string label, string info)
    {
        ...
        Verse.Widgets.Label(rect1, label);
        Verse.Widgets.Label(rect2, info);
    ...
    public static void DrawThingRow(Thing thing)
    {
        ...
        else if (numLines % 2 == 1) Verse.Widgets.DrawLightHighlight(rect);

        rect = new Rect(24f, num + 12f + 1f, 22f, 22f);
        if (thing is Blueprint || thing is Frame)
            Verse.Widgets.DefIcon(rect, thing.def);
        else if (thing is Pawn || thing is Corpse)
            Verse.Widgets.ThingIcon(rect.ExpandedBy(5f), thing);
        else
            Verse.Widgets.ThingIcon(rect, thing);

        rect = new Rect(58f, num + 12f, 370f, 24f);
        Verse.Widgets.Label(rect, thing.LabelMouseover);
    ...
    {
        GUI.color = Color.gray;
        Verse.Widgets.DrawLineHorizontal(curXY.x, curXY.y + Padding, width);
