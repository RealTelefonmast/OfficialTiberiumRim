...
public class NetIOConfig : Editable
{
    ...
    [Unsaved()] public IntVec2 patternSize = IntVec2.Invalid;
    ...
    {
        if (pattern != null) pattern = Regex.Replace(pattern, @"[\s|]+", "");

        if (patternSize.IsValid) TLog.Warning("Pattern size should not be set manually.");
        ...
        if (patternSize.IsInvalid && def != null)
        {
            ...
            if (pattern != null && pattern.Length != patternSize.Area)
                TLog.Warning(
                    $"Pattern is not the same size as the area of the thing ({pattern.Length} != {patternSize.Area}), this will cause an error.");
        }
