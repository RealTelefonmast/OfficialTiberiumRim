...
public static partial class TWidgets
{
    ...
    {
        private static readonly Regex HEXREGEX = new("^#([0-9A-Fa-f]{0,6})$");
        ...
        public static string TextField(Rect rect, string text, int maxLength = -1, Regex? inputValidator = null)
        {
            ...
            if (inputValidator == null || inputValidator.IsMatch(result)) return result;
            return text;
        }
            ...
            string? context = null)
        {
            ...
            var inputRes = GUI.TextField(rect, "#" + buffer, 7);
        ...
        {
            var pattern = Regex.Escape(format).Replace(@"\{0\}", "(.*)");
            var match = Regex.Match(formatted, pattern);
            return match.Success ? match.Groups[1].Value : formatted;
        }

        public static bool TextFieldNumeric<T>(Rect rect, ref T value, float min = 0, float max = float.MaxValue,
            GUIStyle? style = null, string? context = null) where T : unmanaged
        {
            ...
            if (fieldValue != buffer && Widgets.IsPartiallyOrFullyTypedNumber(ref value, fieldValue, min, max))
            {
                buffer = fieldValue;
