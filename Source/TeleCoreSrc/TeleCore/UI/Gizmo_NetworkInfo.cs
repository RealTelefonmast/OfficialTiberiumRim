...
public class Gizmo_NetworkInfo : Gizmo
{
    ...
    private readonly int gizmoPadding = 5;
    private readonly int mainWidth = 200;
    private readonly NetworkComponent parentComp;
    private readonly int selSettingHeight = 22;
    ...
    public Gizmo_NetworkInfo(NetworkComponent parent)
    {
        order = -250f;
        parentComp = parent;
        if (HasExtension)
        ...
        HasSubValues = parentComp.Props.networkRoles.Any(n => n.HasSubValues);
        ...
        var POS = new Vector2(0, currentInspectTab.PaneTopY);
        var SIZE = currentInspectTab.RequestedTabSize;

        var MAINSIZE = new Vector2(mainWidth + gizmoPadding, SIZE.y);
        var BGRECT = new Rect(POS.x, POS.y, MAINSIZE.x, MAINSIZE.y);
        var MAINRECT = BGRECT.AtZero().ContractedBy(5f);

        //Popout Settings
        var SETTINGS_Rect = new Rect(BGRECT.x, BGRECT.y - MAINSIZE.y, BGRECT.width, BGRECT.height);
        ...
        ;
        var SETTINGS_CloseButtonRect = new Rect(SETTINGS_Rect.x + SETTINGS_XOffsetAlignedCenter,
            SETTINGS_Rect.y - SETTINGS_CloseButtonSize.y, SETTINGS_CloseButtonSize.x,
            SETTINGS_CloseButtonSize.y + gizmoPadding);

        var nextY = MAINRECT.y;

        //WidgetRow if available
        var WIDGETROW_Rect = Rect.zero;
        if (HasSubValues)
        {
        ...
        //Title
        var TITLE_Size = Text.CalcSize(cachedStrings[0]);
        var TITLE_Rect =
            new Rect(new Vector2(MAINRECT.x, nextY), TITLE_Size); // CONTENT_Rect.TopPartPixels(TITLE_Size.y);
        nextY = TITLE_Rect.yMax;

        var CONTENT_Rect = MAINRECT.BottomPartPixels(MAINRECT.height - nextY);

        var ROLETEXT_Size = new Vector2(MAINRECT.width / 2, Text.CalcHeight(cachedStrings[1], MAINRECT.width / 2));
        var ROLETEXT_Rect = new Rect(CONTENT_Rect.x + ROLETEXT_Size.x, TITLE_Rect.y, ROLETEXT_Size.x, ROLETEXT_Size.y);

        //Container Readout
        var ContainerGroupRect = CONTENT_Rect.BottomPartPixels(26).LeftHalf();
        var CONTAINER_Rect = ContainerGroupRect.BottomPartPixels(16);
        var REQUESTSELECTION_Rect = ContainerGroupRect.TopPartPixels(10);
        ...
        var height = 2 * width;
        //Designators
        var DESIGNATORS_Rect = new Rect(CONTENT_Rect.xMax - width, CONTENT_Rect.yMax - height, width, height);
    ...
    private void Tick()
    {
        ...
        if (Math.Abs(currentExtendedY - desiredY) > 0.01)
        {
            ...
            currentExtendedY = Mathf.Clamp(currentExtendedY + val * extensionSettings.Count,
                extensionSettingYRange.TrueMin, extensionSettingYRange.TrueMax);
        }
    }
    ...
    public override GizmoResult GizmoOnGUI(Vector2 topLeft, float maxWidth, GizmoRenderParms parms)
    {
        ...
        Find.WindowStack.ImmediateWindow(GetHashCode(), setRect, WindowLayer.GameUI, delegate
        {
    ...
    private void DrawSettingSelection()
    {
        ...
        foreach (var setting in extensionSettings)
        {
            ...
            if (Widgets.ButtonInvisible(labelRect)) selectedSetting = setting.Key;
    ...
    private void DrawMainContent(Rect rect)
    {
        ...
        if (HasSubValues)
        {
            var subFunctionRow = new WidgetRow();
        ...
        if (parentComp.NetworkRole.HasFlag(NetworkRole.Requester))
        {
            ...
            if (Widgets.ButtonText(selectorRect, parentComp.RequesterMode.ToString()))
            {
                ...
                {
                    new(RequesterMode.Automatic.ToString(),
                        delegate { parentComp.RequesterMode = RequesterMode.Automatic; }),
            ...
            Widgets.DrawLineHorizontal(requestArrowRect.x, requestArrowRect.y + requestArrowRect.height / 2,
                requestArrowRect.width);
            TWidgets.DrawBarMarkerAt(requestArrowRect, getVal);
            var setVal =
                (float)Math.Round(
                    GUI.HorizontalSlider(requestArrowRect, getVal, 0.1f, 1f, GUIStyle.none, GUIStyle.none),
                    1); // Widgets.HorizontalSlider(requestArrowRect, getVal, 0, 1f, true, roundTo: 0.01f);
        ...
        {
            //
            var containerRect = UILayout["ContainerRect"];
    ...
    private void DrawSelectedSetting()
    {
        ...
        Find.WindowStack.ImmediateWindow(parentComp.GetHashCode(), settingRect, WindowLayer.GameUI, delegate
        {
    ...
    private void SetExtensions()
    {
        ...
        if (parentComp.NetworkRole.HasFlag(NetworkRole.Requester))
            extensionSettings.Add("Requester Settings", delegate(Rect rect)
            {
                ...
                {
                    var typeRect = new Rect(curX, contentRect.height - 15, 10, 10);
                    var typeSliderSetting = new Rect(curX, contentRect.height - (20 + 100), 10, 100);
                    var typeFilterRect = new Rect(curX, typeSliderSetting.y - 10, 10, 10);
                    ...
                    //
                    var newValue =
                        TWidgets.VerticalSlider(typeSliderSetting, previousValue, 0, Container.Capacity, 0.01f);
        ...
        if (parentComp.HasContainer)
            extensionSettings.Add("Container Settings", delegate(Rect rect)
            {
                ...
                {
                    var menu = new FloatMenu(RightClickFloatMenuOptions.ToList(), cachedStrings[2], true);
                    menu.vanishIfMouseDistant = true;
                    Find.WindowStack.Add(menu);
                }
                //
            ...
            extensionSettings.Add("Filter Settings", delegate(Rect rect)
            {
                ...
                Widgets.BeginScrollView(scrollOutRect, ref filterScroller, scrollViewRect, false);
                {
                    ...
                    foreach (var acceptedType in parentComp.Container.AcceptedTypes)
                    {
                        ...
                        listingScroll.CheckboxLabeled(
                            $"{acceptedType.LabelCap.CapitalizeFirst().Colorize(acceptedType.valueColor)}: ",
                            ref boolVal);
                ...
                //Paste Option
                var clipBoard =
                    ClipBoardUtility.TryGetClipBoard<Dictionary<NetworkValueDef, bool>>(RWStringCache
                        .NetworkFilterClipBoard);
                var clipBoardInsertActive =
                    clipBoard != null && clipBoard.All(t => Container.AcceptedTypes.Contains(t.Key));
