...
{
    private readonly HashSet<IntVec3> thinRoofCells = new();
    private Vector3 actualCenter;
    public List<PollutionConnector> connections = new();
    private Vector3 drawPos;
    private int markedDirty, width, height;

    public Dictionary<RoomComponent_Pollution, List<PollutionConnector>> neighbourConnections = new();
    ...
    public HashSet<IntVec3> BorderCellsNoCorners { get; } = new();

    public List<PollutionConnector> Connections =>
        UsesOutDoorPollution ? PollutionInfo.ConnectionsToOutside : connections;


    public PollutionContainer Outside => PollutionInfo.OutsideContainer;
    public PollutionContainer ActualContainer { get; private set; }
    ...
    public override void Create(RoomTracker parent)
    {
        ...
        Log.Message("Creating " + Group.ID + " (Now " + PollutionInfo.AllComps.Count + ")");

        //Create new comps
        ActualContainer = new PollutionContainer();
        ...
        if (PollutionInfo.Cache.TryGetAverageRoomPollution(Group, out int pollution))
    ...
    {
        Log.Message("Disbanding " + Group.ID);
        var pollInfo = map.Tiberium().PollutionInfo;
        pollInfo.Notify_DisbandedComp(this);
        foreach (var connector in connections) pollInfo.Notify_RemoveConnection(connector);
    ...
    public override void Notify_RoofClosed()
    {
        ...
        ActualContainer.RegenerateData(Group.CellCount);
    }
    ...
    public void Equalize()
    {
        ...
        if (ShouldPushToOther(ActualSaturation, Outside.Saturation))
        {
            ...
            TryPushToOther(ref from, ref to,
                PushAmountToOther(ActualSaturation, Outside.Saturation,
                    TiberiumPollutionMapInfo.CELL_CAPACITY * Parent.OpenRoofCount));
    ...
    public void Notify_FlowChanged()
    {
        ...
        {
            Log.Message($"Trying to regen 0-sized room! {this.Group.ID}");
            return;
        }
    }
    ...
    {
        Log.Message("Regenerating " + Group.ID + " " + Map);
        ...
        if (!onlyConnections || !BorderCellsNoCorners.Any())
        {
            int minX = Group.Cells.MinBy(t => t.x).x;
            int maxX = Group.Cells.MaxBy(t => t.x).x;
            int minZ = Group.Cells.MinBy(t => t.z).z;
            int maxZ = Group.Cells.MaxBy(t => t.z).z;
            width = maxX - minX + 1;
            height = maxZ - minZ + 1;
            ...
            renderer.UpdateMesh(Group.Cells, MinVec, width, height);
        }

        foreach (var connector in connections) PollutionInfo.Notify_RemoveConnection(connector);
    ...
    private void GenerateCellData()
    {
        ...
        foreach (IntVec3 c in Group.Cells)
        {
            ...
            for (var i = 0; i < 4; i++)
            {
                ...
                if (region == null || region.Room.Group != Group) BorderCellsNoCorners.Add(cardinal);
            }
        }
    }
    ...
    public override void OnGUI()
    {
        if (Find.CameraDriver.CurrentZoom == CameraZoomRange.Closest)
        {
            ...
            GenMapUI.DrawThingLabel(v, Group.ID + "[" + Pollution + "]{" + UsedContainer.TotalCapacity + "}",
                Color.red);
        }
        //vectorField.OnGUI();
    }
    ...
    {
        if (!UsesOutDoorPollution) renderer.Draw(drawPos, Saturation);
        //vectorField.DrawVectors();
        if (Group.Cells.Contains(UI.MouseCell()))
        {
            GenDraw.DrawFieldEdges(Group.Cells.ToList(), Color.cyan);
            ...
            if (Connections.Any())
            {
                ...
                GenDraw.DrawFieldEdges(Connections.SelectMany(t => t.Other(this).Group.Cells).ToList(), Color.red);
            }
        }

        var vec = Group.Cells.First().ToVector3();
