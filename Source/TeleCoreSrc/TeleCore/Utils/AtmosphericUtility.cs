...
public static class AtmosphericUtility
{
    ...
    {
        if (!Verse.UI.MouseCell().InBounds(Find.CurrentMap)) return;
        ...
        rect.x += 26f;
        rect.y += 26f;
        if (rect.xMax > Verse.UI.screenWidth) rect.x -= rect.width + 52f;

        if (rect.yMax > Verse.UI.screenHeight) rect.y -= rect.height + 52f;
