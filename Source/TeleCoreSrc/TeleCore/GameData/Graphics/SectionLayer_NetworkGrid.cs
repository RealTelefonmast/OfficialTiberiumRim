...
public class SectionLayer_NetworkGrid : SectionLayer_Things
{
    ...
    public override void Regenerate()
    {
        ...
        {
            List<Thing> list = TeleCore.Map.thingGrid.ThingsListAt(item);
            var count = list.Count;
            for (var i = 0; i < count; i++)
            {
                var thing = list[i];
                if ((thing.def.seeThroughFog ||
                     !TeleCore.Map.fogGrid.fogGrid[CellIndicesUtility.CellToIndex(thing.Position, TeleCore.Map.Size.x)]) &&
                    thing.def.drawerType != 0 &&
                    (thing.def.drawerType != DrawerType.RealtimeOnly || !requireAddToMapMesh) &&
                    (!(thing.def.hideAtSnowDepth < 1f) ||
                     !(TeleCore.Map.snowGrid.GetDepth(thing.Position) > thing.def.hideAtSnowDepth)) &&
