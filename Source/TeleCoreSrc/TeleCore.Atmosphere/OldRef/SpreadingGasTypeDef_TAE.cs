...
namespace TeleCore.Atmosphere.OldRef
{
    public class SpreadingGasTypeDef_TAE : Def
    {
        ...
        [Unsaved]
        private TAE.AtmosphericTransferWorker workerInt;
        [Unsaved]
        ...
        public TAE.AtmosphericTransferWorker TransferWorker => workerInt ??= (TAE.AtmosphericTransferWorker)Activator.CreateInstance(transferWorker, this);
        public float ViscosityMultiplier { get; private set; }

        public static implicit operator ushort(SpreadingGasTypeDef_TAE def) => def.IDReference;
        public static explicit operator SpreadingGasTypeDef_TAE(int ID) => _defByID[ID];
