...
public class TiberiumContent
{
    ...
    public static readonly Texture2D LockedBanner = ContentFinder<Texture2D>.Get("UI/Menu/LockedBanner");

    public static readonly Texture2D TopBar = ContentFinder<Texture2D>.Get("UI/Menu/TopBar");
    public static readonly Texture2D TibOptionBG = ContentFinder<Texture2D>.Get("UI/Menu/TibOptionBG");
    public static readonly Texture2D TibOptionBG_Cut = ContentFinder<Texture2D>.Get("UI/Menu/TibOptionBG_Cut");
    public static readonly Texture2D TibOptionBG_CutFade = ContentFinder<Texture2D>.Get("UI/Menu/TibOptionBG_CutFade");
    ...
    public static readonly Texture2D SideBarArrow = ContentFinder<Texture2D>.Get("UI/Menu/Arrow");

    public static readonly Texture2D HighlightAtlas = ContentFinder<Texture2D>.Get("UI/Widgets/TutorHighlightAtlas");
    public static readonly Texture2D HightlightInMenu = ContentFinder<Texture2D>.Get("UI/Icons/HighLight");

    //Research
    public static readonly Texture2D UnseenResearch = ContentFinder<Texture2D>.Get("UI/Icons/Research/UnseenResearch");
    public static readonly Texture2D NewResearch = ContentFinder<Texture2D>.Get("UI/Icons/Research/NewResearch");

    //
    public static readonly Texture2D OpenMenu = ContentFinder<Texture2D>.Get("UI/Icons/Research/OpenMenu");
    public static readonly Texture2D Construct = ContentFinder<Texture2D>.Get("UI/Icons/Research/Construct");
    public static readonly Texture2D SelectThing = ContentFinder<Texture2D>.Get("UI/Icons/Research/SelectThing");

    //UI - Icons
    //--Controls
    //----Harvester
    public static readonly Texture2D HarvesterRefinery = ContentFinder<Texture2D>.Get("UI/Icons/Harvester/NewRefinery");
    public static readonly Texture2D HarvesterReturn = ContentFinder<Texture2D>.Get("UI/Icons/Harvester/Return");
    public static readonly Texture2D HarvesterHarvest = ContentFinder<Texture2D>.Get("UI/Icons/Harvester/Harvest");
    public static readonly Texture2D HarvesterValue = ContentFinder<Texture2D>.Get("UI/Icons/Harvester/Value");
    public static readonly Texture2D HarvesterNearest = ContentFinder<Texture2D>.Get("UI/Icons/Harvester/Nearest");
    public static readonly Texture2D HarvesterMoss = ContentFinder<Texture2D>.Get("UI/Icons/Harvester/Moss");

    public static readonly Texture ZoneCreate_HarvestTiberium =
        ContentFinder<Texture2D>.Get("UI/Icons/Zones/ZoneCreate_HarvestTiberium");

    public static readonly Texture ZoneCreate_HarvestTiberium_Producer =
        ContentFinder<Texture2D>.Get("UI/Icons/Zones/ZoneCreate_HarvestTiberium_Producer");

    //----SuperWeapon
    public static readonly Texture2D NodNukeIcon = ContentFinder<Texture2D>.Get("UI/Icons/SuperWeapon/Launch_Nuke");

    public static readonly Texture2D IonCannonIcon =
        ContentFinder<Texture2D>.Get("UI/Icons/SuperWeapon/Launch_IonCannon");

    public static readonly Texture2D FireStorm_On = ContentFinder<Texture2D>.Get("UI/Icons/SuperWeapon/Firestorm_On");
    public static readonly Texture2D FireStorm_Off = ContentFinder<Texture2D>.Get("UI/Icons/SuperWeapon/Firestorm_Off");

    //Harvester Bar
    public static readonly Material Harvester_EmptyBar =
        SolidColorMaterials.NewSolidColorMaterial(new Color(0.3f, 0.3f, 0.3f, 0.65f), ShaderDatabase.MetaOverlay);

    public static readonly Material Harvester_FilledBar =
        SolidColorMaterials.NewSolidColorMaterial(new Color(0f, 1f, 1f, 1f), ShaderDatabase.MetaOverlay);

    public static readonly Material RedMaterial =
        SolidColorMaterials.NewSolidColorMaterial(new Color(1f, 0f, 0f, 1f), ShaderDatabase.MetaOverlay);

    public static readonly Material GreenMaterial =
        SolidColorMaterials.NewSolidColorMaterial(new Color(0f, 1f, 0f, 1f), ShaderDatabase.MetaOverlay);

    public static readonly Material BlueMaterial =
        SolidColorMaterials.NewSolidColorMaterial(new Color(0f, 0f, 1f, 1f), ShaderDatabase.MetaOverlay);

    public static readonly Material ClearMaterial =
        SolidColorMaterials.NewSolidColorMaterial(new Color(0f, 0f, 0f, 0f), ShaderDatabase.MetaOverlay);

    //----Tib Container
    public static readonly Texture2D ContainMode_Sludge =
        ContentFinder<Texture2D>.Get("UI/Icons/Container/ContainMode_Sludge");

    public static readonly Texture2D ContainMode_TripleSwitch =
        ContentFinder<Texture2D>.Get("UI/Icons/Container/ContainMode_Storage");
    ...
    public static readonly Texture2D
        TiberiumIcon_Base = ContentFinder<Texture2D>.Get("UI/Icons/Tiberium/Tiberium_Base");

    //ThingCategories
    public static readonly Texture2D TiberiumIcon = ContentFinder<Texture2D>.Get("UI/Icons/Tiberium/Tiberium_RGB");

    //Tiberium Network
    public static readonly Texture2D Network_MissingConnection =
        ContentFinder<Texture2D>.Get("UI/Icons/Network/ConnectionMissing");
    ...
    public static void PreGenerateFlowMapVectors()
    {
        ...
        var size = pixelDensity * intSize.x;

        var pixels = new Color[size * size];
        var pixels2 = new Color[size * size];
        for (var p = 0; p < pixels.Length; p++) pixels[p] = pixels2[p] = new Color(0.5f, 0.5f, 0, 1);

        var cells = new CellRect(0, 0, intSize.x, intSize.z);
    ...
    {
        for (var x = 0; x < pixelDensity; x++)
        for (var z = 0; z < pixelDensity; z++)
        {
            var partX = (inv ? -1f : 1f) / pixelDensity;
            var partZ = (inv ? -1f : 1f) / pixelDensity;
            var pixelFlow = new Vector3(cellFlow.x + (partX + x * partX), 0, cellFlow.z + (partZ + z * partZ));

            var vec = new Vector2(pixelFlow.x, pixelFlow.z);
            vec.Normalize();

            var dist = Mathf.Abs(Vector3.Distance(origin, pixelFlow));
            var decay = Mathf.InverseLerp(radius, 0f, dist);
            vec *= decay;

            var r = Mathf.InverseLerp(-1, 1, vec.x);
            var g = Mathf.InverseLerp(-1, 1, vec.y);

            var pixelX = cell.x * pixelDensity + x;
            var pixelZ = cell.z * pixelDensity + z;
            var index = pixelX + pixelZ * size;
    ...
    public static Color[] GetSection(Color[] pixels, int startX, int startY, int width, int height, int oldSize)
    {
        ...
        {
            var oldIndex = startX + x + (startY + z) * oldSize;
            var newIndex = x + z * width;
    ...
    {
        var pixelsFinal = new Color[pixels.Length];
        for (var i = 0; i < pixels.Length; i++) pixelsFinal[i] = pixels[i];
    ...
    {
        var newTex = new Texture2D(size.x, size.z, TextureFormat.RGBAFloat, false);
    ...
    private static Color[] RotateRight(Color[] pixels, IntVec2 size, out IntVec2 newSize, bool invert = false)
    {
        ...
        for (var z = 0; z < size.z; z++)
        {
            var colorIndex = size.x - 1 - x + z * size.x;
            var transposedIndex = z + x * size.z;
    ...
    private static Color[] RotateLeft(Color[] pixels, IntVec2 size, out IntVec2 newSize)
    {
        ...
        for (var z = 0; z < size.z; z++)
        {
            var colorIndex = x + (size.z - 1 - z) * size.x;
            var transposedIndex = z + x * size.z;
