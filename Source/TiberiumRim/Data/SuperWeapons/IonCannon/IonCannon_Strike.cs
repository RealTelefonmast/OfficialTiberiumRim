...
public class IonCannon_Strike : ThingWithComps
{
    ...
    public override void SpawnSetup(Map map, bool respawningAfterLoad)
    {
        ...
        if (!respawningAfterLoad)
        {
            ...
            composition.AddPart(delegate { DoSetup(composition.CurrentTick); }, TRFDefOf.IonCannon_StartUp,
            ...
            composition.AddPart(delegate
            {
                MakeIonBubble(5, radius, new ColorInt(70, 90, 175).ToColor, TRFDefOf.IonDistortionBubble);
                MakeIonBubble(5, radius * 0.4f, new ColorInt(100, 120, 175).ToColor, TRFDefOf.IonCenterDistortionBubble);

            }, 7f); // 5 second alive time for bubble
            composition.AddPart(TRFDefOf.IonCannon_PreClimaxPause,
            SoundInfo.InMap(posInfo, MaintenanceType.PerFrame), 8f);
            composition.AddPart(TRFDefOf.IonCannon_ClimaxChargeUp,
            ...
            composition.AddPart(delegate
            {
                IonBeam beam = (IonBeam)ThingMaker.MakeThing(TRFDefOf.IonBeam);
                ...
                var fleck = FleckMaker.GetDataStatic(DrawPos, Map, TRFDefOf.IonBeamBurn, 6.5f * radius);
                fleck.rotationRate = 1.2f;
                Map.flecks.CreateFleck(fleck);

                ActionComposition ionExpComp = new ActionComposition("Ion Last Exp");
                Mote distortion = (Mote)ThingMaker.MakeThing(TRFDefOf.IonExplosionShockwave);
    ...
    private void MakeIonBubble(float time, float bubbleRadius, Color color, ThingDef ionDistortion)
    {
        ...
        Mote mote = (Mote)ThingMaker.MakeThing(TRFDefOf.IonBubble);
