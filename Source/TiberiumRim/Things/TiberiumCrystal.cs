...
public class TiberiumCrystal : TiberiumThing
{
    ...
    public bool InGrowRange
    {
        get
        {
            if (HasParent && inRange > 0) return Position.DistanceTo(parent.Position) <= inRange;
            return true;
        }
    }
    ...
    public TiberiumLifeStage LifeStage
    {
        get
        {
            ...
            if (Growth > 0.85f) return TiberiumLifeStage.Spreading;
            if (Growth > 0.26f) return TiberiumLifeStage.ProducingTerrain;
    ...
    public int HarvestTime
    {
        get
        {
            var val = def.tiberium.harvestTime.SecondsToTicks() * Growth;
    ...
    protected float GrowthPerTick => growthPerTick * GrowthRate;
        ...
        if ((!parent?.NoTerrain ?? true) && LifeStage == TiberiumLifeStage.ProducingTerrain)
        {
            ...
            if (!(terrain is TiberiumTerrainDef))
                if (GenTiberium.AnyCorruptedOutcomes(def, terrain, out TerrainSupport support))
                    Map.terrainGrid.SetTerrain(Position, support.TerrainOutcome);
            updatedTerrain = true;
        }
    ...
    public bool CanBeHarvestedBy(TR.Harvester harvester)
    {
    ...
    public override void Print(SectionLayer layer)
    {
        ...
        foreach (var num5 in positionIndices)
        {
            ...
            var vector2 = new Vector2(num3, num3);
            var center = vector;
            var size = vector2;
            var mat = matSingle;
            var flipUv = @bool;
            Printer_Plane.PrintPlane(layer, center, size, mat, 0f, flipUv, null, new Color32[4], 0.1f,
                this.HashOffset() % 1024);
            num4++;
            if (num4 >= num) break;
        }
    ...
    public override string GetInspectString()
    {
        ...
        if (Dormant) sb.AppendLine("DormantTiberium".Translate());
        sb.AppendLine("PercentGrowth".Translate(Growth.ToStringPercent()));
        if (Growth < 1f) sb.AppendLine("Growth".Translate() + ": " + GrowthRate.ToStringPercent());
        if (HarvestableNow) sb.AppendLine("HarvestValue".Translate() + ": " + Mathf.RoundToInt(HarvestValue));
