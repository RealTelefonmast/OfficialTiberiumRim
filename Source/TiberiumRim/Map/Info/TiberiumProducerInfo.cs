...
{
    private readonly BoolGrid producerBoolGrid;
    private readonly TiberiumProducer[] producerFieldGrid;
    private readonly List<TiberiumProducer> producers = new();
    ...
    public IEnumerable<TiberiumField> TiberiumFields =>
        producers.NullOrEmpty() ? default : producers.Select(t => t.TiberiumField);

    public IEnumerable<TiberiumField> MarkedFields => TiberiumFields.EnumerableNullOrEmpty()
        ? default
        : TiberiumFields.Where(t => t.MarkedForFastGrowth);

    public IEnumerable<TiberiumCrystal> TiberiumCrystals => MarkedFields.EnumerableNullOrEmpty()
        ? default
        : MarkedFields.SelectMany(t => t.GrowingCrystals);

    public bool ShouldSpread =>
        !MarkedFields.EnumerableNullOrEmpty(); //TiberiumFields.Any(t => t?.MarkedForFastGrowth ?? false);
    ...
    public override void Tick()
    {
        ...
        if (crystalIterator == null && CanReset)
            ...
            for (var i = 0; i < 20; i++)
            {
                ...
                if (current?.Spawned ?? false) current.TickLong();
