...
{
    //Infection
    private readonly int[] infectionIntGrid;

    //General
    private readonly Map map;

    //Radiation
    private readonly int[] radiationIntGrid;
    private readonly List<IRadiationSource> RadiationSources = new();
    ...
    public CellBoolDrawer RadiationDrawer => drawerInt ??= new CellBoolDrawer(RadiationDrawerBool, RadiationDrawerColor,
        RadiationDrawerExtraColor, map.Size.x, map.Size.z, 3605, 0.25f);
    ...
    private bool RadiationDrawerBool(int index)
    {
        ...
        var passed = !intVec.Fogged(map) && RadiationAt(intVec) > 0f;
        return passed;
    }
    ...
    {
        var intVec = CellIndicesUtility.IndexToCell(index, map.Size.x);
        var num = RadiationAt(intVec);
        var color = Color.Lerp(Color.green, Color.red, num);
        if (num > 1)
    ...
    public void Update()
    {
        if (TRUtils.Tiberium().GameSettings.RadiationOverlay) RadiationDrawer.MarkForDraw();
    ...
    public void Notify_SourceDespawned(IRadiationSource source)
    {
        ...
        foreach (var cell in source.AffectedCells)
        {
            if (!cell.InBounds(map) ||
                Enumerable.Any(RadiationSources, s => s != source && s.AffectsCell(cell))) continue;
    ...
    {
        return TRUtils.InverseLerpUnclamped(0, 255, RadiationIntAt(pos));
    }
    ...
    {
        return TRUtils.InverseLerpUnclamped(0, 255, InfectionIntAt(pos));
    }
