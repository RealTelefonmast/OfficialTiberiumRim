...
public class ChronoVortex : PortalSpawner
{
    ...
    {
        var randomCell1 = Position + GenRadial.RadialPattern[TRUtils.Range(0, GenRadial.NumCellsInRadius(2.75f))];

        var from = Position;
        var to = randomCell1;

        var normed = (to - from).ToVector3().normalized;
        var toCell = from + (normed * Rand.Range(6, 8)).ToIntVec3();

        var arc = (Mote_Arc)ThingMaker.MakeThing(TiberiumDefOf.Mote_Arc);
        var mat = MaterialsTesla.Arcs[TRUtils.Range(0, 3)];
        ...
        arc.Attach(null);
        GenSpawn.Spawn(arc, Position, Map);
        SoundDef.Named("ElectricArc").PlayOneShot(SoundInfo.InMap(this));
    }
}