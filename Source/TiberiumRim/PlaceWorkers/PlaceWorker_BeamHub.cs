    ...
    */
    ...
    private List<IntVec3> CellsForRot(Building_BeamHub mainHub, Rot4 direction, int length)
    {
        ...
        for (var i = 1; i <= length; i++)
        {
            ...
            if (thing != null && thing != mainHub &&
                (thing is Building || thing is Building_BeamHub || thing is Building_BeamHubSegmentPart)) break;
            cells.Add(curCell);
        }
    ...
    private void DrawSegmentGhost(ThingDef def, IntVec3 center, Rot4 dir, int length)
    {
        ...
        for (var i = 1; i <= length; i++)
        {
            ...
            TDrawing.Draw(GhostUtility.GhostGraphicFor(def.graphicData.Graphic, def, Color.cyan),
                curCell.ToVector3Shifted(), Rot4.North, 0, null, null);
        }
    }
    ...
    public override void DrawGhost(ThingDef def, IntVec3 center, Rot4 rot, Color ghostCol, Thing thing = null)
    {
        ...
        foreach (var otherPart in Find.CurrentMap.listerThings.ThingsOfDef(def))
            for (var i = 0; i < 4; i++)
            {
                ...
                var otherPos = MinOrOther(otherPart.Position, center,
                    otherPart.Position + GenAdj.CardinalDirections[i] * trDef.beamHub.range, i);
