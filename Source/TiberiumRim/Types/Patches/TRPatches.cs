...
internal static class TRPatches
{
    ...
    public static class MassUtility_CapacityPatch
    {
        ...
        {
            return value + p.GetStatValue(TRCDefOf.ExtraCarryWeight);
        }
    }
    ...
    private static class AutoHomeAreaMakerNotify_BuildingClaimedPatch
    {
        public static void Postfix(Thing b)
        {
            if (b != null) TRUtils.ResearchCreationTable().TryTrackConstructedOrClaimedBuilding(b.def);
        }
    }
    ...
    private static class Designator_Build_FinalizeSuccPatch
    {
        ...
        {
            if (__instance is Designator_Cancel) GameComponent_EVA.EVAComp().ReceiveSignal(EVASignal.Cancelled, null);
        }
    }
    ...
    private static class Designator_Build_FinalizeFailPatch
    {
        public static void Postfix(Designator __instance)
        {
            if (__instance is Designator_Build) GameComponent_EVA.EVAComp().ReceiveSignal(EVASignal.CantDeploy, null);
        }
    }
