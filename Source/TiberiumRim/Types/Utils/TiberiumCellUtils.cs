...
public static class TiberiumCellUtils
{
    public static bool AllowTiberiumMeteorite(IntVec3 x, Map map)
    {
        ...
        if (map.Tiberium().StructureInfo.AllProducers.Any(p => p.Position.DistanceTo(x) < 46)) return false;
        ...
        foreach (var c in GenAdj.CellsOccupiedBy(x, Rot4.North, new IntVec2(5, 5)))
        {
            ...
            if (!terrain.affordances.Contains(TiberiumDefOf.TiberiumResearchCrane.terrainAffordanceNeeded))
                return false;
