...
{
    public static Toil CarryHauledThingToCell(LocalTargetInfo targetInfo,
        PathEndMode pathEndMode = PathEndMode.ClosestTouch)
    {
        var toil = new Toil();
        ...
        toil.AddFailCondition(delegate
        {
            ...
            var cell = targetInfo.Cell;
            return actor.jobs.curJob.haulMode == HaulMode.ToCellStorage &&
                   !cell.IsValidStorageFor(actor.Map, actor.carryTracker.CarriedThing);
        });
        return toil;
    }

    public static Toil PlaceHauledThingInCell(LocalTargetInfo targetInfo, Toil nextToilOnPlaceFailOrIncomplete,
            ...
            if (!actor.carryTracker.TryDropCarriedThing(cell, ThingPlaceMode.Direct, out thing, placedAction))
            {
                if (storageMode)
                {
                    ...
                    if (nextToilOnPlaceFailOrIncomplete != null &&
                        ((tryStoreInSameStorageIfSpotCantHoldWholeStack && StoreUtility.TryFindBestBetterStoreCellForIn(
                             actor.carryTracker.CarriedThing, actor, actor.Map, StoragePriority.Unstored, actor.Faction,
                             cell.GetSlotGroup(actor.Map), out c)) ||
                         StoreUtility.TryFindBestBetterStoreCellFor(actor.carryTracker.CarriedThing, actor, actor.Map,
                             StoragePriority.Unstored, actor.Faction, out c)))
                    {
                        if (actor.CanReserve(c)) actor.Reserve(c, actor.CurJob);
                    ...
                    Log.Warning(string.Format(
                        "Incomplete haul for {0}: Could not find anywhere to put {1} near {2}. Destroying. This should be very uncommon!",
                        actor, actor.carryTracker.CarriedThing, actor.Position));
    ...
    {
        var gotoDest = new Toil();
        gotoDest.initAction = delegate { gotoDest.actor.pather.StartPath(targetInfo.Thing, PathEndMode.Touch); };
        gotoDest.AddFailCondition(delegate
        {
            var thing = targetInfo.Thing;
            if (thing.Destroyed ||
            ...
            return thingOwner != null && !thingOwner.CanAcceptAnyOf(gotoDest.actor.carryTracker.CarriedThing);
        });
