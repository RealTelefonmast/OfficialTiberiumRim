...
{
    public List<IntVec3> ConnectedCells = new();
    //public List<>

    public List<TiberiumNetworkBuilding> FullList = new();
    public TiberiumNetworkBuilding parent;
    public List<TNW_Pipe> Pipes = new();
    public List<TNW_Refinery> Refineries = new();
    public List<TNW_Silo> Silos = new();
            ...
            if (cell.GetFirstBuilding(map) is TiberiumNetworkBuilding tnwb && tnwb.CanConnectTo(cell, parent))
                if (!FullList.Contains(tnwb))
                {
                    if (tnwb is TNW_Pipe pipe)
                    {
                        ...
                        {
                            Pipes.Add(pipe);
                            pipe.UpdateDirectParent(parent,
                                GenAdj.CellsAdjacentCardinal(pipe).Where(c => parent.ConnectableCells.Contains(c))
                                    .First());
                            pipe.UpdatePipeMode();
                        }
                        else
                    ...
                    if (tnwb is TNW_Silo silo) Silos.Add(silo);
                    if (tnwb is TNW_Refinery refinery) Refineries.Add(refinery);
                    if (flag)
                    {
    ...
    {
        foreach (var tnwb in other.FullList)
        {
            if (newNet != null) tnwb.Network = newNet;
            AddNewStructure(tnwb);
        }
    }

    public void AddNewStructure(TiberiumNetworkBuilding tnwb, IntVec3 newPos = new())
    {
        if (!FullList.Contains(tnwb))
        {
            if (tnwb is TNW_Pipe pipe)
    ...
    public void RemoveStructure(TiberiumNetworkBuilding tnwb)
    {
        if (FullList.Contains(tnwb))
        {
            if (tnwb is TNW_Pipe pipe) Pipes.Remove(pipe);
            if (tnwb is TNW_Silo silo) Silos.Remove(silo);
            if (tnwb is TNW_Refinery refinery) Refineries.Remove(refinery);
    ...
    public override string ToString()
    {
        ...
        sb.AppendLine("Pipes: ");
        foreach (TNW_Pipe pipe in Pipes) sb.AppendLine("   - " + pipe);
        sb.AppendLine("Silos: ");
        foreach (var silo in Silos) sb.AppendLine("   - " + silo);
        sb.AppendLine("Refineries: ");
        foreach (var refinery in Refineries) sb.AppendLine("   - " + refinery);
        return sb.ToString();
    }
}