...
public class JobDriver_RepairDroneRepair : JobDriver
{
    ...
    private MechanicalPawn Target => TargetA.Thing as MechanicalPawn;
    private List<Hediff> Hediffs => (job as JobWithExtras).hediffs;
    ...
    public override IEnumerable<Toil> MakeNewToils()
    {
        ...
        this.FailOn(() =>
            !DroneStation.FuelComp.HasFuel ||
            Target.Position.DistanceTo(DroneStation.parent.Position) > DroneStation.Props.radius);
        var gotoTarget = Toils_Goto.GotoThing(TargetIndex.A, PathEndMode.Touch);
        yield return gotoTarget;
        var repair = new Toil();
        repair.initAction = delegate { Drone.pather.StopDead(); };
        repair.tickAction = delegate
        {
            ...
            repair.actor.jobs.EndCurrentJob(JobCondition.Succeeded);
        };
        repair.WithEffect(TargetThingA.def.repairEffect, TargetIndex.A);
        repair.defaultCompleteMode = ToilCompleteMode.Never;
        repair.JumpIf(
            () => !GetActor().CanReachImmediate(GetActor().jobs.curJob.GetTarget(TargetIndex.A), PathEndMode.Touch),
            gotoTarget);
        yield return repair;
    }
}