...
{
    [Unsaved] private IntVec3 targetPos = IntVec3.Invalid;
    ...
    private Thing ResearchThing => TargetThingA;
        ...
        get
        {
            ...
            else
            {
                ...
                bool Predicate(IntVec3 x)
                {
                    ...
                    var distanceTo = TargetA.Cell.DistanceTo(x);
                    if (distanceTo > maxDistance || distanceTo < minDistance) return false;
                    return GenSight.LineOfSight(TargetA.Cell, x, Map);
                }

                CellFinder.TryFindRandomCellNear(TargetA.Cell, Map, Mathf.CeilToInt(distance), Predicate,
                    out targetPos);
    ...
    public override IEnumerable<Toil> MakeNewToils()
    {
        ...
        yield return PathEndMode == PathEndMode.InteractionCell
            ? Toils_Goto.GotoCell(TargetIndex.A, PathEndMode)
            : Toils_Goto.GotoCell(TargetPos, PathEndMode);

        var task = Project.CurrentTask;
        var research = new Toil();
        research.tickAction = delegate
        {
            var pawn = research.actor;
            float num = 1;
            if (task.RelevantPawnStat != null)
                num *= pawn.GetStatValue(task.RelevantPawnStat);
            if (task.RelevantTargetStat != null)
                num *= TargetThingA.GetStatValue(task.RelevantTargetStat);
            Manager.PerformResearch(task, pawn, num);
            if (!task.SkillRequirements.NullOrEmpty())
                foreach (var skillReq in task.SkillRequirements)
                    pawn.skills.GetSkill(skillReq.skill).Learn(0.11f);
