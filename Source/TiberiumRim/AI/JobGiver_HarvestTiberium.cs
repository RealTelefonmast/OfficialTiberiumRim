...
public class JobDriver_HarvestTiberium : JobDriver
{
    ...
    private TR.Harvester Harvester => pawn as TR.Harvester;

    private bool FailOn => Harvester.idleAtRefinery || (TiberiumCrystal.def.IsMoss
        ? Harvester.harvestMode != TR.HarvestMode.Moss
        : Harvester.harvestMode == TR.HarvestMode.Moss);
    ...
    {
        string report = TiberiumCrystal.def.IsMoss
            ? "EradicateMoss".Translate()
            : "HarvestingReport".Translate(TargetA.Thing.def.LabelCap);
        return "HarvestingReport".Translate(TargetA.Thing.def.LabelCap);
    }
    ...
    protected override IEnumerable<Toil> MakeNewToils()
    {
        ...
        {
            Log.Message(TiberiumCrystal.HarvestTime + " | " + TiberiumCrystal.HarvestValue + " | " +
                        TiberiumCrystal.Growth);
            ...
            growthPerValue = TiberiumCrystal.Growth / ticksPerValue;
                ...
                if (ticksPassed < ticksToHarvest)
                {
                    if (ticksPassed % ticksPerValue == 0)
                        if (Harvester.Container.TryAddValue(TiberiumCrystal.def.TiberiumType, 1, out int excess))
                            TiberiumCrystal.growth -=
                                growthPerValue - excess / TiberiumCrystal.def.tiberium.harvestValue;
                }
                else
                {
                    TiberiumCrystal.DeSpawn();
