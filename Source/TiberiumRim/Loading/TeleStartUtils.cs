...
internal static class TeleStartUtils
{
    ...
    {
        var defDataBase = typeof(DefDatabase<>);
        var idStack = typeof(DefIDStack);
        ...
        foreach (var defType in typeof(Def).AllSubclasses())
            try
            {
                ...
                var defDataBase_AllDefsListForReading = defDataBase_OfDef.GetProperty("AllDefsListForReading",
                ...
                foreach (var defObject in data)
                    try
                    {
                        ...
                        if (defObject != defObjectFromID)
                        {
                            ...
                            TLog.Warning(
                                $"Checking {defObject} failed: ({defObject}){defID} != ({defObjectFromID}){subDefID}");
                        }
                    }
                    catch (Exception e)
                    {
    ...
    internal static void ExecuteDefInjectors()
    {
        ...
        foreach (var injectorType in injectorTypes) TLog.Debug(" - " + injectorType.Name);
