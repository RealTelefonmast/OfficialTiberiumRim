...
{
    [TweakValue("Patches_MSThreshold")] public static int MSThreshold = 25;
    ...
    */
    ...
    public static class TestPatch
    {
        ...
        {
            var helper = AccessTools.Method(typeof(TestPatch), nameof(GibHelp));
    ...
    [StaticConstructorOnStartup]
    private class FirstGame
    {
        [HarmonyPostfix]
        private static void Fix()
        {
            var mod = TiberiumRimMod
                .mod; //LoadedModManager.ModHandles.FirstOrDefault(x => x is TiberiumRimMod) as TiberiumRimMod;
    ...
    public static class TileInspectString_Patch
    {
        ...
        {
            var level = TRUtils.Tiberium().TiberiumInfo.CoverageAt(Find.WorldSelector.selectedTile);
    ...
    public static class Hediff_PostAddPatch
    {
        public static void Postfix(Hediff __instance)
        {
            ...
            if (__instance.pawn.Faction?.IsPlayer ?? false) TRUtils.EventManager().CheckForEventStart(__instance);
        }
    }
    ...
    public class JobDriverWait_CheckForAutoAttackPatch
    {
        ...
        {
            if (__instance.pawn.Downed) return;
            if (__instance.pawn.Faction != null && __instance.job.def == JobDefOf.Wait_Combat &&
    ...
    public static class Pawn_AddDraftedOrdersPatch
    {
        public static void Postfix(Vector3 clickPos, Pawn pawn, List<FloatMenuOption> opts)
        {
            ...
            foreach (LocalTargetInfo attackTarg in GenUI.TargetsAt_NewTemp(clickPos,
                         TargetingParameters.ForAttackHostile(), true, null))
            {
                var rangedAct = HediffRangedHelper.GetRangedAttackAction(pawn, attackTarg, out var str);
                ...
                else
                {
                    ...
                    {
                        MoteMaker.MakeStaticMote(attackTarg.Thing.DrawPos, attackTarg.Thing.Map,
                            ThingDefOf.Mote_FeedbackShoot);
                        rangedAct();
                    };
                }
    ...
    public static class HediffDrawerPatch
    {
        ...
        {
            var mutation = (Hediff_TiberiumMutation)diffs.FirstOrDefault(h => h is Hediff_TiberiumMutation);
    ...
    public static class MassUtility_CapacityPatch
    {
        ...
        {
            return value + p.GetStatValue(TiberiumDefOf.ExtraCarryWeight);
        }
    }
    ...
    public static class StatOffsetFromGearPatch
    {
        public static float Postfix(float value, Thing gear, StatDef stat)
        {
            ...
            return value +
                   ((List<ConditionalStatModifier>)trDef.conditionalStatOffsets).GetStatOffsetFromList(stat, pawn);
        }
    }
    ...
    public static class SpecialDisplayStatsPatch
    {
        public static IEnumerable<StatDrawEntry> Postfix(IEnumerable<StatDrawEntry> values, ThingDef __instance)
        {
            ...
            if (__instance is TRThingDef trDef)
                ...
                {
                    var verb = (VerbProperties_TR)__instance.Verbs.First(x => x.isPrimary);
                }
    ...
    private static class Designator_Build_FinalizeSuccPatch
    {
        ...
        {
            if (__instance is Designator_Cancel) GameComponent_EVA.EVAComp().ReceiveSignal(EVASignal.Cancelled);
        }
    }
    ...
    public static class SetRoofPatch
    {
        public static void Postfix(RoofGrid __instance, IntVec3 c)
        {
            ...
            if (suppression.IsInSuppressionCoverage(c, out var sups)) suppression.MarkDirty(sups);
        }
    }
    ...
    private static class CompleteConstructionPatch
    {
    ...
    private static class BillDonePatch
    {
        public static void Postfix(Pawn billDoer, List<Thing> products)
        {
            //Construction Task Logic
            foreach (var product in products) TRUtils.ResearchCreationTable().TryTrackCreated(product);
        }
    }
    ...
    internal static class RecipeDef_AvailableNowPatch
    {
        ...
        {
            var TRRequisiteDone =
                __instance.products.All(t => (t.thingDef as TRThingDef)?.requisites?.FulFilled() ?? true);
    ...
    public static class SpawnSetupPatch
    {
        public static void Postfix(Thing __instance)
        {
            ...
            if (__instance is Building building)
            {
                ...
                {
                    var sources = radiation.RadiationSourcesAt(building.Position);
                    foreach (var source in sources) source.Notify_BuildingSpawned(building);
                }

                if (!building.CanBeSeenOver())
                {
                    var suppression = building.Map.Tiberium().SuppressionInfo;
                    if (suppression.IsInSuppressionCoverage(building.Position, out var sups))
    ...
    public static class DeSpawnPatch
    {
        ...
        public static void Postfix(Thing __instance)
        {
            ...
            if (updateSuppressionGrid)
            {
                ...
                if (suppression.IsInSuppressionCoverage(instancePos, out var sups)) suppression.MarkDirty(sups);
            }
            ...
            if (updateRadiationGrid)
            {
                ...
                if (radiation.IsInRadiationSourceRange(instancePos))
                {
                    var sources = radiation.RadiationSourcesAt(instancePos);
                    foreach (var source in sources)
                    {
    ...
    public static class GlowPatch
    {
        public static bool Prefix(CompGlower __instance, Map map)
        {
            ...
            {
                map.mapDrawer.MapMeshDirty(__instance.parent.Position, MapMeshFlagDefOf.Things);
    ...
    public static class PlaySettingsPatch
    {
        public static void Postfix(WidgetRow row, bool worldView)
        {
            if (!worldView)
            {
                row.ToggleableIcon(ref TRUtils.GameSettings().EVASystem, TiberiumContent.Icon_EVA,
                    "Enable or disable the EVA", SoundDefOf.Mouseover_ButtonToggle);
                row.ToggleableIcon(ref TRUtils.GameSettings().RadiationOverlay, TiberiumContent.Hediff_Radiation,
                    "Toggle the Tiberium Radiation overlay.", SoundDefOf.Mouseover_ButtonToggle);
            }
        }
    }
    ...
    public static class MakeNewBillPatch
    {
        public static void Postfix(ref Bill __result)
        {
            ...
            {
                var tibBill = new TiberiumBill(__result.recipe as TRecipeDef);
                __result = tibBill;
            }
        }
    }
    ...
    public static class ThingSetMaker_Meteorite_ResetPatch
    {
        public static void Postfix()
        {
            Traverse.Create(typeof(ThingSetMaker_Meteorite)).Field("nonSmoothedMineables").GetValue<List<ThingDef>>()
                .RemoveAll(t => t is TRThingDef);
        }
    }
    ...
    public static class MapInterfaceUpdatePatch
    {
        public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            var helper = AccessTools.Method(typeof(MapInterfaceUpdatePatch), nameof(TiberiumMapInterfaceUpdate));
            var updateCall = AccessTools.Method(typeof(DeepResourceGrid), "DeepResourceGridUpdate");
