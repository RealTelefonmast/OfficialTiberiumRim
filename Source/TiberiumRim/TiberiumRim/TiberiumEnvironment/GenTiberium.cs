...
namespace TiberiumRim
{
    public static class GenTiberium
    {
        ...
        {
            bool Predicate(IntVec3 c) => TrySpawnTiberium(c, crystal.Map, crystal.def, crystal.Parent);

            /*
            if (CellFinder.TryFindRandomCellNear(crystal.Position, crystal.Map, (int) crystal.def.props.spreadRadius, predicate, out IntVec3 result))
            {
                return true;
            }
            */
        ...
        //Used when attempting to spawn props at a random position
        public static bool TrySpawnTiberium(IntVec3 pos, Map map, TiberiumCrystalDef def, TiberiumProducer parent = null)
        {
            ...
            //Check if the props can spread to the position
        ...
        public static bool TryMutatePlant(Plant plant, TiberiumCrystalDef def)
        {
            ...
            //if (!TRUtils.Chance(def.props.plantMutationChance)) return false;
            var map = plant.Map;
