...
namespace TiberiumRim
{
    public static class TRWidgets
    {
        ...
        public static void DrawTextureInCorner(Rect rect, Texture2D texture, TextAnchor anchor)
        {
            Rect newRect = new Rect();
            switch (anchor)
            {
                case TextAnchor.UpperLeft:
                    newRect = new Rect(rect.x, rect.y, texture.width, texture.height);
                    break;
                case TextAnchor.LowerLeft:
                    newRect = new Rect(rect.x, rect.yMax - texture.height, texture.width, texture.height);
                    break;
                case TextAnchor.UpperRight:
                    newRect = new Rect(rect.xMax - texture.width, rect.y, texture.width, texture.height);
                    break;
                case TextAnchor.LowerRight:
                    newRect = new Rect(rect.xMax - texture.width, rect.yMax - texture.height, texture.width, texture.height);
                    break;
            }
            Widgets.DrawTextureFitted(newRect, texture, 1f);
        }
