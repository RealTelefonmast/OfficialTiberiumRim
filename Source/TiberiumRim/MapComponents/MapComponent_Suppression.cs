...
{
    private readonly List<Comp_Suppression> dirtySuppressors = new();
    ...
    public List<Comp_Suppression> Suppressors = new();
    ...
    {
        foreach (var suppressor in Suppressors) UpdateGrid(suppressor);
    }
    ...
    {
        foreach (var cell in cells) grid.Set(cell, false);
    }

    public void AddToGrid(List<IntVec3> cells)
    {
        foreach (var cell in cells) grid.Set(cell, true);
    }
