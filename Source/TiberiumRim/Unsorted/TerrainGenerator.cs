...
public static class TerrainGenerator
{
        ...
        Predicate<IntVec3> validator, Action<IntVec3> action)
    {
        ...
        var growthsDone = new bool[rootCount];
        var rootPositions = new bool[8] { true, true, true, true, true, true, true, true };
        ...
        for (var i = 0; i < rootCount; i++)
        {
            ...
            while (!growthsDone[i])
            {
                ...
                RETRY2:
                currentAttempts++;
                var currentDistance = lastCell.DistanceTo(root);
    ...
    {
        var currentDistance = start.DistanceTo(end);
        var densityCurve = Mathf.Lerp(0.7f, 0, currentDistance / radius);
    ...
    public static float WeightByPushNPull(IntVec3 checkingCell, IntVec3 lastCell, IntVec3 pusher, IntVec3 puller)
    {
        ...
        var pushWeight = pusher.IsValid ? checkingCell.DistanceTo(pusher) - lastCell.DistanceTo(pusher) : 0;
        var pullWeight = puller.IsValid ? lastCell.DistanceTo(puller) - checkingCell.DistanceTo(puller) : 0;
    ...
    //Get Cell
    public static bool GetNextRandomCell(IntVec3 from, IntVec3 pusher, List<IntVec3> visited,
        Predicate<IntVec3> validator, Action<IntVec3> action, out IntVec3 nextCell)
    {
    ...
    public static IEnumerable<IntVec3> RandomRootPatch(CellRect root, Map map, float radius, int rootCount,
        Predicate<IntVec3> validator, Action<IntVec3> action)
    {
        var visitedCells = new List<IntVec3>();
        var doneParts = new bool[root.EdgeCellsCount];

        var pusher = root.CenterCell;

        var currentCell = IntVec3.Invalid;
        var lastCell = IntVec3.Invalid;
        ...
        foreach (var rootCell in root.EdgeCells)
        {
            ...
            while (!doneParts[i])
            {
                ...
                lastCell = currentCell;
                RETRY2:
                var currentDistance = lastCell.DistanceTo(pusher);
                if (currentDistance > radius || currentAttempts > 100)
                {
                    doneParts[i] = true;
                    break;
                ...
                EXTRACELL:
                lastCell.CellsAdjacent8Way().TryRandomElementByWeight(
                    w => WeightByDist(pusher, w, radius, Mathf.Clamp(w.DistanceTo(pusher) - currentDistance, 0, 2)),
                    out currentCell);
