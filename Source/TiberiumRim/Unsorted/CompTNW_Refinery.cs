...
{
    private readonly List<TR.Harvester> harvesters = new();
    ...
    public IntVec3 PositionFor(TR.Harvester harvester)
    {
        var slot = parkingZone.SlotFor(harvester);
        if (!slot.IsValid || parkingZone == null)
            return parent.InteractionCell;
        return slot;
    }

    private TR.Harvester SpawnNewHarvester()
    {
        TR.Harvester harvester = (TR.Harvester)PawnGenerator.GeneratePawn(Props.harvester, parent.Faction);
        ...
        return (TR.Harvester)GenSpawn.Spawn(harvester, spawnLoc, parent.Map, parent.Rotation.Opposite);
    }

    public void AddHarvester(TR.Harvester harvester)
    {
    ...
    public void RemoveHarvester(TR.Harvester harvester)
    {
    ...
    public override IEnumerable<Gizmo> CompGetGizmosExtra()
    {
        ...
        foreach (var gizmo in parkingZone.GetZoneAddGizmos()) yield return gizmo;
