...
public class FXGraphic
{
    ...
    public FXGraphic(CompFX parent, FXGraphicData data, int index)
    {
        ...
        if (data.directAltitudeOffset.HasValue)
            ...
            altitude = data.altitude.HasValue
                ? data.altitude.Value.AltitudeFor()
                : parent.parent.def.altitudeLayer.AltitudeFor() + 0.125f * (index + 1);
    ...
    public void Draw(Vector3 drawPos, Rot4 rot, float? rotation, Action<FXGraphic> action, int index)
    {
        ...
        var info = new GraphicDrawInfo(Graphic, drawPos, rot, ((FXThingDef)parent.parent.def).extraData,
            parent.parent.def);
        var mat = info.drawMat;
        ...
        Graphics.DrawMesh(info.drawMesh, new Vector3(info.drawPos.x, altitude, info.drawPos.z),
            rotation?.ToQuat() ?? info.rotation.ToQuat(), mat, 0);
    }
    ...
    public void Print(SectionLayer layer, Vector3 drawPos, Rot4 rot, float? rotation, Thing parent)
    {
        ...
        Printer_Plane.PrintPlane(layer, new Vector3(info.drawPos.x, altitude, info.drawPos.z), info.drawSize,
            info.drawMat, rotation ?? info.rotation, info.flipUV);
    }
}