...
public class VeinholeSystem : IExposable
{
    ...
    {
        //Spread Hub
        if (Rand.MTBEventOccurs(2, GenDate.TicksPerDay, 1)) TrySpreadHub();

        //Spread Egg
        if (CanMakeEgg && Rand.MTBEventOccurs(1, GenDate.TicksPerDay, 1)) TrySpawnEgg();
    }
    ...
    public void TrySpreadHub()
    {
        ...
        {
            var terrain = parent.Ruleset.RandomOutcome(c.GetTerrain(parent.Map));
            if (terrain != null)
                parent.Map.terrainGrid.SetTerrain(c, terrain);
        };

        var end = GenRadial.RadialCellsAround(parent.Position, 56, false).RandomElement();
