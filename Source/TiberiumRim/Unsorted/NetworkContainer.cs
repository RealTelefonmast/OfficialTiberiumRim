...
public class NetworkContainer<T> : IExposable where T : Enum
{
    ...
    private Dictionary<T, float> StoredValues = new();
    private float totalCapacity;
    private Dictionary<T, bool> TypeFilter = new();
    ...
    public NetworkContainer(IContainerHolder<T> parent, float capacity, List<T> acceptedTypes,
        NetworkContainerSet<T> parentSet = null)
    {
        Parent = parent;
        totalCapacity = capacity;
    ...
    public float TotalStorage { get; }

    public float StoredPercent => TotalStorage / totalCapacity;
    ...
    public List<T> AllStoredTypes { get; }
    ...
    public IContainerHolder<T> Parent { get; }
    ...
    {
        var newContainer = new NetworkContainer<T>(Parent);
        ...
        foreach (var type in AllStoredTypes) newContainer.TryAddValue(type, StoredValues[type], out var e);
        return newContainer;
    }
    ...
    public bool TryAddValue(T valueType, float wantedValue, out float actualValue)
    {
        ...
        if (StoredValues.TryGetValue(valueType, out var value))
