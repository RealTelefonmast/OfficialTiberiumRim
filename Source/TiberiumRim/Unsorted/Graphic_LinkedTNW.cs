...
public class Graphic_LinkedTNW : Graphic_Linked
{
    ...
    public override void DrawWorker(Vector3 loc, Rot4 rot, ThingDef thingDef, Thing parent, float extraRotation)
    {
        for (var i = 0; i < 4; i++)
        {
            ...
            if (cell.InBounds(parent.Map))
                Graphics.DrawMesh(MeshAt(rot), cell.ToVector3ShiftedWithAltitude(parent.def.Altitude),
                    Quaternion.identity, LinkedDrawMatFrom(parent, cell), 0);
        }
    }
    ...
    public override void Print(SectionLayer layer, Thing parent)
    {
        ...
        if (parent is TNW_Pipe pipe && !pipe.DirectParent.DestroyedOrNull())
            Printer_Plane.PrintPlane(layer, pipe.ParentCell.ToVector3ShiftedWithAltitude(parent.def.Altitude),
                Vector2.one, LinkedDrawMatFrom(parent, pipe.ParentCell));
    }
}