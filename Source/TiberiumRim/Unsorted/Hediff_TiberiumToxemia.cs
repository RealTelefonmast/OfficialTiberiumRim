...
{
    private static readonly Color mainColor = new ColorInt(0, 255, 125).ToColor;
    private static readonly Color mutationColor = new ColorInt(200, 100, 255).ToColor;
    ...
    public override string LabelInBrackets
    {
        get
        {
            ...
            else
            {
                ...
                if (time != null)
                    label += "TR_HediffToxemiaInfBracket".Translate(
                        time?.ToStringTicksToPeriod(false, false, true, false));
            }
    ...
    private string MutationInfo => "TOX:" + Math.Round(ToxemiaSeverity, 2) + "|MC:" + Math.Round(MutationChance, 4) +
                                   "/" + Math.Round(MTBValue, 2) + "d |BIC:" + Math.Round(BloodInfectionChance, 4);
    ...
    public override bool ShouldRemove
    {
        get
        {
            var tmpList = ListPool<Hediff_CauseToxemia>.Get();
            pawn.health.hediffSet.GetHediffs(ref tmpList);
            var hasAny = tmpList.Count > 0; //!(HasRadiation || HasCrystallization);
            ListPool<Hediff_CauseToxemia>.Release(tmpList);
            return hasAny;
        }
    }

    private float CrystallizedPercent =>
        TotalAffectedParts /
        (float)pawn.kindDef.RaceProps.body.AllParts
            .Count; //(float)affectedPartsCount / pawn.kindDef.RaceProps.body.AllParts.Count;
    ...
    private IEnumerable<Hediff_CrystallizingPart> Crystallizing =>
        pawn.health.hediffSet.GetHediffs<Hediff_CrystallizingPart>();

    private IEnumerable<Hediff_CrystallizedPart> Crystallized =>
        pawn.health.hediffSet.GetHediffs<Hediff_CrystallizedPart>();

    private int TotalAffectedParts => Crystallizing.SelectMany(c => c.Part.AllChildParts(true))
        .Union(Crystallized.SelectMany(c => c.Part.AllChildParts(true))).Count();
    ...
    private float MTBValue => (TicksUntilDeath(pawn)?.TicksToDays() ?? 1f) *
                              Mathf.Lerp(0.65f, 0.2f, Radiation?.Severity ?? 0.5f);
    ...
    private float BloodInfectionChance
    {
        get
        {
            var val = ToxemiaSeverity * CrystallizedPercent;
    ...
    */
    ...
    public static int? TicksUntilDeath(Pawn pawn)
    {
        ...
        while (currentPart != null)
        {
            while (curSeverity < 1)
            {
                var val = Hediff_CrystallizingPart.SeverityPerDayFunc(curSeverity) * 0.00333333341f; //Per Tick
                curSeverity += val;
                totalTicks += 200;
            }
