    ...
    public static class GenerateImpliedDefs_PreResolvePatch
    {
        public static void Postfix()
        {
            ...
            foreach (var def in DefDatabase<TRThingDef>.AllDefs)
            {
                ...
                if (def.Minifiable)
                {
                    var mini = TRUtils.MakeNewBluePrint(def, true, blueprint);
                }

                DirectXmlCrossRefLoader.ResolveAllWantedCrossReferences(FailMode.Silent);
            }

            Log.Message("TRThingDefList - Faction Cats: " + TRThingDefList.Categorized.Keys.Count + " | TRThings: " +
                        TRThingDefList.TotalCount);
        }
    }
}