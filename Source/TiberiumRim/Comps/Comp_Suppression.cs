...
public class Comp_Suppression : ThingComp
{
    ...
    public SuppressionMapInfo Suppression { get; set; }
    public CompPowerTrader PowerComp { get; set; }
    public List<IntVec3> SuppressionCells { get; set; } = new();
    ...
    public override void PostSpawnSetup(bool respawningAfterLoad)
    {
        ...
        Suppression = parent.Map.Tiberium().SuppressionInfo;
    ...
    public void UpdateSuppressionCells()
    {
        ...
        {
            return !c.Roofed(parent.Map) && GenSight.LineOfSight(parent.Position, c, parent.Map);
        }

        SuppressionCells = TRUtils.SectorCells(parent.Position, parent.Map, Props.radius, Props.angle,
            parent.Rotation.AsAngle, false, Predicate).ToList();
    }
