...
public class VisceralPod : TRThing, IThingHolder
{
    ...
    private Pawn InnerPawn
    {
        get
        {
            if (HeldThing is Corpse corpse) return corpse.InnerPawn;
    ...
    public VisceralStage VisceralStage
    {
        get
        {
            ...
            {
                case null:
                    return this.VisceralStage.Empty;
                //Fresh Pawn
                case Pawn p when !(p is TiberiumPawn):
                    return this.VisceralStage.Fresh;
                //Not so fresh Pawn
                case Corpse _ when RottenPercent < 1f:
                    return this.VisceralStage.Corpse;
            }

            //Jesus christ thats not even a pawn
            if (ticksLeft > 0) return this.VisceralStage.Horror;
            //Welp, now we got these
            return this.VisceralStage.Visceroid;
        }
    }
    ...
    public override void Tick()
    {
        ...
        {
            case this.VisceralStage.Fresh:
                DoBurnDamage();
                break;
            case this.VisceralStage.Corpse:
                Rot();
                break;
            case this.VisceralStage.Horror:
                ...
                break;
            case this.VisceralStage.Visceroid:
                InnerContainer.Clear();
                MakeVisceroids();
                if (!CanOpen) return;
                Open();
    ...
    private void DoBurnDamage()
    {
        ...
        {
            //Inside
            part = pawn.HealthComp().OrgansInside
                .Where(p => pawn.health.hediffSet.PartHasHediff(p, TRHediffDefOf.ViscousBloat)).RandomElement();
            if (part == null) return;
