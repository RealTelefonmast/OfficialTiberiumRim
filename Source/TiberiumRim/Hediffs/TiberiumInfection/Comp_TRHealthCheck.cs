...
{
    public int NonMisingPartsCount;
    public List<BodyPartRecord> NonMissingParts = new();
    public List<BodyPartRecord> OrgansInside = new();

    public List<BodyPartRecord> OutsideParts = new();
    public List<BodyPartRecord> PartsForGas = new();
    public List<BodyPartRecord> PartsForInfection = new();
    public List<BodyPartRecord> PartsForMutation = new();
    ...
    private void DoRadiationClick(float rads)
    {
        MoteMaker.ThrowText(parent.TrueCenter() + new Vector3(0.5f, 0f, 0.5f), parent.Map,
            ((float)Math.Round(rads, 3)).ToString("0.###"), Color.white);
    ...
    {
        var veinCorpse = pawn.InnerPawn.VeinCorpseDef();
        var corpse = (VeinholeFood)ThingMaker.MakeThing(veinCorpse);
    ...
    public void UpdateParts()
    {
        ...
        PartsForGas = NonMissingParts.Where(p =>
                p.def.tags.Any(t => t == BodyPartTagDefOf.BreathingPathway || t == BodyPartTagDefOf.BreathingSource))
            .ToList();
        PartsForInfection = NonMissingParts
            .Where(p => p.height == BodyPartHeight.Bottom && p.depth == BodyPartDepth.Outside).ToList();
        ;
...
public class CompProperties_TiberiumCheck : CompProperties
{
    ...
    {
        compClass = typeof(Comp_TRHealthCheck);
    }
}