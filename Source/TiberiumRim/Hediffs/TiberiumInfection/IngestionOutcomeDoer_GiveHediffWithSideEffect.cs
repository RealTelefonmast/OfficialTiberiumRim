...
public class IngestionOutcomeDoer_GiveHediffWithSideEffect : IngestionOutcomeDoer_GiveHediff
{
    ...
    private bool ShouldDoSideEffect(Pawn pawn)
    {
        var gotNeeds = pawn.needs.AllNeeds.Any(x => needs.Contains(x.def));
        gotNeeds = needsNeeded ? !gotNeeds : gotNeeds;

        var gotHediffs = hediffs.Any(h => pawn.health.hediffSet.HasHediff(h));
