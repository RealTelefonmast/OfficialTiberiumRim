...
public class Hediff_CrystallizingPart : Hediff_CauseToxemia
{
    ...
    private bool Reversing => pawn.health.hediffSet.HasHediff(TRHediffDefOf.TiberiumImmunity) ||
                              pawn.health.hediffSet.HasHediff(TRHediffDefOf.TiberBlockHediff);

    private bool CanFuseToGround =>
        Part.IsInGroup(BodyPartGroupDefOf.Legs) && CurrentStage == InfectionStage.Crystallizing;
    ...
    public override bool TryMergeWith(Hediff newHediff)
    {
        ...
        if (newHediff.Part.IsAncestorOf(Part))
        {
        ...
        if (newHediff.Part.IsChildOf(Part))
        {
    ...
    private void Crystallize()
    {
        ...
        var crystallized = (Hediff_MissingPart)HediffMaker.MakeHediff(TRHediffDefOf.CrystallizedPart, pawn);
