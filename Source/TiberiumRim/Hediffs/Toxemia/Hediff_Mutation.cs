...
{
    private bool symbioticBias;
    private bool visceralBias;

    public override string LabelInBrackets =>
        pawn.Dead
            ? null
            : (VisceralPct + SymbioticPct).ToStringPercent(); //VisceralRisk.ToStringPercent() + "/x" + MutationSpeed;
    ...
    private IEnumerable<Hediff_TiberiumMutationPart> MutationParts =>
        pawn.health.hediffSet.GetHediffs<Hediff_TiberiumMutationPart>();

    private bool CanFinalize =>
        (Mathf.RoundToInt(VisceralPct * 100f) + Mathf.RoundToInt(SymbioticPct * 100)) / 100 == 1;

    private int VisceralParts =>
        MutationParts.Count(p => p.Mutation == Hediff_TiberiumMutationPart.MutationState.Visceral);

    private int SymbioticParts =>
        MutationParts.Count(p => p.Mutation == Hediff_TiberiumMutationPart.MutationState.Symbiotic);
    ...
    public int MutationSpeed
    {
        get
        {
            ...
            return Mathf.RoundToInt(Mathf.Lerp(1, 8,
                1 - pawn.health.hediffSet.GetFirstHediffOfDef(TRHediffDefOf.TiberiumToxemia).Severity));
        }
    }
    ...
    private void MutateRandomNewPart()
    {
        ...
        var mutationPart =
            (Hediff_TiberiumMutationPart)HediffMaker.MakeHediff(TRHediffDefOf.TiberiumMutationPart, pawn);
        var symbioticChance = 1 - VisceralRisk;
        var visceral = TRUtils.Chance(VisceralRisk);
        if (visceral)
        {
            ...
            if (TRUtils.Chance(VisceralRisk * VisceralRisk))
            {
                ...
                var hediff = TRHediffDefOf.Visceral.HediffFor(randomPart);
                if (hediff != null) pawn.health.AddHediff(hediff, randomPart);
            }
        }
        else
        {
            ...
            {
                //
                var hediff = TRHediffDefOf.Symbiotic.HediffFor(randomPart);
                if (hediff != null) pawn.health.AddHediff(hediff, randomPart);
            }
        }
    ...
    {
        var visceral = VisceralPct > SymbioticPct;
        ...
        foreach (var hediff in partList) pawn.health.RemoveHediff(hediff);
    ...
    private void ApplyImmunity()
    {
        ...
        for (var index = pawn.health.hediffSet.hediffs.Count - 1; index >= 0; index--)
        {
            ...
            {
                var crystalDiff = TRHediffDefOf.Crystallized.HediffFor(hediff.Part);
                if (crystalDiff != null)
                {
    ...
    {
        var viscNum = VisceralPct;
        var symbNum = SymbioticPct;
        var reverseVisc = 1f - viscNum;
        ...
        var fillRectTotal = new Rect(0f, curY, rect.width, 18f).ContractedBy(3);
        var visceral = fillRectTotal.LeftHalf();
        var symbiotic = fillRectTotal.RightHalf();
        var fullArea = new Rect(fillRectTotal.xMin, rect1.yMin, fillRectTotal.width,
            fillRectTotal.height + rect1.height);
