...
public class Projectile_Homing : ProjectileTR, IPatchedProjectile
{
    ...
    private Vector3 PushVelocity
    {
        get
        {
            var osci = TMath.OscillateBetween(-WiggleValue, WiggleValue, OSC_TICKS, startTick + tickOffset) *
                       Mathf.Clamp01(1f - DistanceCoveredFraction * 2);
            return Vector3.Scale((PushDirection.normalized + new Vector3(osci, 0, 0)) * speed * 0.75f,
                new Vector3(8 * Mathf.Clamp01(DistanceCoveredFraction * 3), 1, 1)) * PushStrength;
        }
    }
    ...
    public override bool PreLaunch(Thing launcher, Vector3 origin, LocalTargetInfo usedTarget,
        ...
        Thing equipment = null, ThingDef targetCoverDef = null)
    {
        ...
        var sign = TRandom.Chance(0.5f) ? -1 : 1;
