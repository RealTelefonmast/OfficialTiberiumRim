...
namespace TR
{
    ...
    public static class StaticData
    {
        ...
        public static void Notify_NewTibMapComp(MapComponent_Tiberium mapComp)
        {
            ...
            FlowMapsByMap[mapComp.map.uniqueID] =
                new RenderTexture(pixelDensity * map.Size.x, pixelDensity * map.Size.z, 0);
        }

        //Data Accessors
        public static T GetDesignatorFor<T>(ThingDef def) where T : Designator
        {
            if (CachedDesignators.TryGetValue(def, out var des)) return (T)des;
        ...
        public static Color[] PixelCanvas(int size, int pixelDensity)
        {
            ...
            for (var i = 0; i < newCanvas.Length; i++) newCanvas[i] = Color.clear;
            if (CanvasBySize.TryGetValue(size, out _)) CanvasBySize.Remove(size);
