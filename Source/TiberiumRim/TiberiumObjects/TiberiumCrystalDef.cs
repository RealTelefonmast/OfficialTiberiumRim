...
public class TiberiumCrystalDef : TRThingDef
{
    ...
    private static readonly Dictionary<int, TiberiumCrystalDef> _defByID = new();
    ...
    //
    [Unsaved] private float? growthPerTick;

    [Unsaved] public int IDReference;
    [Unsaved] private IEnumerable<IntVec3> radialCells;

    public TiberiumCrystalProperties tiberium;

    public float GrowthPerTick =>
        growthPerTick ??= 1f / (GenDate.TicksPerDay * tiberium.growDays / GenTicks.TickLongInterval);

    public IEnumerable<IntVec3> SpreadRangeMask =>
        radialCells ??= GenRadial.RadialPatternInRadius(tiberium.spreadRadius);
    ...
    public bool TryGetTiberiumOutcomesAt(IntVec3 pos, Map map, out TerrainDef topTerrain, out TerrainDef underTerrain,
        out TiberiumCrystalDef crystalDef)
    {
        conversions.GetOutcomes(pos, map, out topTerrain, out underTerrain, out crystalDef);
        return crystalDef != null;
    }

    public void GetTiberiumOutcomesAt(IntVec3 pos, Map map, out TerrainDef topTerrain, out TerrainDef underTerrain,
        out TiberiumCrystalDef crystalDef)
    {
        conversions.GetOutcomes(pos, map, out topTerrain, out underTerrain, out crystalDef);
    }
