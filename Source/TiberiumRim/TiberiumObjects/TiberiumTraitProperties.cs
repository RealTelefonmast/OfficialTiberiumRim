...
public class TiberiumTraitProperties : IExposable
{
    ...
    public float Growth => Mathf.Clamp(growth, 0.001f, 2f);

    public float Spread => Mathf.Clamp(spread, 0f, 2f);

    public float HarvestYield => Mathf.Clamp(harvestYield, 0.1f, 2f);

    public float HarvestTime => Mathf.Clamp(harvestTime, 0.25f, 2f);

    public float Radiation => Mathf.Clamp(radiation, 0f, 2f);

    public float Infectivity => Mathf.Clamp(infectivity, 0f, 2f);

    public float Evolution => Mathf.Clamp(evolution, 0.01f, 2f);

    public float Damage => Mathf.Clamp(damage, 0f, 2f);
    ...
    public void Evolve(TiberiumCrystal crystal)
    {
        ...
        var totalPoints = TotalPoints(crystal);
        var general = totalPoints / offsets.Length;
        for (var i = 0; i < offsets.Length; i++)
        {
            var offset = totalPoints > 0f ? general : 0f;
            totalPoints -= offset;
            if (TRUtils.RandValue > 0.4646469f) offset = -offset;
    ...
    {
        var total = TRUtils.Range(0.7f, 1.4f);
