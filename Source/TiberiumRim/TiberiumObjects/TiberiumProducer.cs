...
public class TiberiumProducer : TRBuilding
{
    ...
    protected bool IsMature =>
        (areaMutator?.Finished ?? true) || areaMutator.ProgressPct >= def.spawner.minProgressToSpread;
    ...
    private void SpawnTiberium(bool forceSpawn = false)
    {
        ...
        switch (def.spawner.spawnMode)
        {
            ...
            case TiberiumSpawnMode.SporeBurst:
                ...
                for (var i = 0; i < spores; i++)
                {
                    var tibDef = TiberiumCrystalDefWeighted;
                    CellFinder.TryFindRandomCellNear(Position, Map, (int)def.spawner.sporeExplosionRadius,
                        c => c.CanSendSporeTo(Map, tibDef), out var dest);
