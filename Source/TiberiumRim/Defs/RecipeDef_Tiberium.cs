...
{
    private readonly Dictionary<TiberiumValueType, float> specifics = new();
    public float cost;
    public List<TiberiumTypeCost> costs = new();
    ...
    public IEnumerable<TiberiumValueType> SpecificTypes =>
        costs.Where(c => c.cost > 0).Select(c => c.TiberiumValueType);
    ...
    public bool CanPay(TiberiumContainer container)
    {
        ...
        if (specTypes.Any())
        {
            ...
            if (totalCost - anyCost != 0) return false;
        }
...
public class TiberiumTypeCost
{
    ...
    public void LoadDataFromXmlCustom(XmlNode xmlRoot)
    {
        ...
        {
            TiberiumValueType =
                (TiberiumValueType)ParseHelper.FromString(xmlRoot.FirstChild.Value, typeof(TiberiumValueType));
        }
        else
        {
