...
public class VisceralPod : TRThing, IThingHolder
{
    ...
    public void Open()
    {
        ...
        Map.mapDrawer.MapMeshDirty(Position, MapMeshFlagDefOf.Buildings, true, false);
    }
    ...
    private Pawn_Visceral CreateHorror(Pawn pawn)
    {
        ...
        if (pawn.Faction?.IsPlayer ?? false)
            Messages.Message("TR.Visceral.Pod.ConversionMessage".Translate(pawn.Name.ToStringShort),
    ...
    private void MakeFilth()
    {
        ...
        {
            var filth = Rand.Element(ThingDefOf.Filth_CorpseBile, ThingDefOf.Filth_Vomit,
    ...
    public override string GetInspectString()
    {
        ...
        if (!hatched)
        {
            ...
            switch (VisceralStage)
            {
                case VisceralStage.Fresh:
                    append +=
                        $"{"Dying".Translate()} - {(1 - InnerPawn.health.summaryHealth.SummaryHealthPercent).ToStringPercent()}";
                    break;
                case VisceralStage.Corpse:
