...
public class MapDefInjector : DefInjectBase
{
    ...
    {
        if (def is MapGeneratorDef) return true;
        return false;
    }
...
{
    //
    private static List<IntVec3> tmpMapCells = new();
    private static FastPriorityQueue<IntVec3> tmpInfestableCells;
    private static readonly List<IntVec3> tmpInfestedCells = new();
    ...
    public static void InfestMapPercent(Map map, float coveragePct, int globSize = 1000)
    {
        ...
        if (coveragePct == totalCoverage) return;

        //TODO CHANGE UP WITH TIB SPECIFIC
        var allInfestableCells = map.pollutionGrid.AllPollutableCells;
        ...
        {
            var num3 = Mathf.FloorToInt(allInfestableCells.Count * coveragePct);
            var num4 = Mathf.CeilToInt(num3 / globSize);
            TRLog.Debug($"Making {num4} globs for a total of {num3} cells");
            for (var j = 0; j < num4; j++)
            {
                var num5 = Mathf.Min(globSize, num3);
        ...
        float GlobCellSelectionWeight(IntVec3 c)
        {
            ...
            return 1f * (c.DistanceTo(map.Center) / map.Size.LengthHorizontal) *
                   (c.DistanceToEdge(map) / map.Size.LengthHorizontal);
        }
    }
    ...
    public static void GrowTiberiumInfestationAt(IntVec3 root, Map map, int cellsToInfest = 4)
    {
        ...
        var crystalDef = Rand.Chance(0.05f) ? TiberiumDefOf.TiberiumBlue : TiberiumDefOf.TiberiumGreen;
        ...
        if (tmpInfestedCells.Count == 0) return;

        tmpInfestableCells.Clear();
        for (var i = 0; i < tmpInfestedCells.Count; i++)
            foreach (var adjacentPollutableCell in GetAdjacentInfestableCells(tmpInfestedCells[i], map))
        ...
        {
            var intVec = tmpInfestableCells.Pop();
        ...
        IEnumerable<IntVec3> GetAdjacentInfestableCells(IntVec3 c, Map m)
        {
            ...
            {
                var intVec3 = c + t;
                if (intVec3.InBounds(map) && intVec3.Walkable(map)) yield return intVec3;
            }
        }
    }
