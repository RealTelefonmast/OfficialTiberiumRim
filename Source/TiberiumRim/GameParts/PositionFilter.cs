...
public class PositionFilter
{
    ...
    public List<DefValue<TerrainDef>> terrainToPrefer = new();
    ...
    public IEnumerable<IntVec3> AllCells(Map map)
    {
        foreach (var cell in map.AllCells)
        {
            ...
            if (Enumerable.Any(terrainToPrefer) &&
                !Enumerable.Any(terrainToPrefer, ttp => TRUtils.Chance(ttp.Value))) continue;
            if (Enumerable.Any(thingsToSpawnAt) &&
                !Enumerable.Any(thingsToSpawnAt, t => cell.GetFirstThing(map, t) != null)) continue;
            if (Enumerable.Any(distanceToThings) && Enumerable.Any(distanceToThings, t =>
                    map.listerThings.ThingsOfDef(t.ThingDef).Any(t2 => t2.Position.DistanceTo(cell) < t.value)))
                continue;
            yield return cell;
        }
    }
    ...
    public IEnumerable<IntVec3> NeededCellsFor(Map map, List<ThingDef> expectedThings)
    {
        ...
        for (var i = 0; i < expectedThings.Count; i++)
