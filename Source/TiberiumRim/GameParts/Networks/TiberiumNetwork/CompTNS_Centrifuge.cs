...
public class SpeedController
{
    ...
    //
    private readonly SecondOrderSpeed secondOrder;
    ...
    public void Update()
    {
        var acceleration = (targetSpeed - curSpeed) * Mathf.Lerp(friction, 1, Mathf.Lerp(0, maxSpeed, curSpeed));
...
{
    private readonly float _d; // Constants used in the algorithm
    private readonly float _w; // Constants used in the algorithm
    private readonly float _z; // Constants used in the algorithm
    private readonly float k1; // Constants used in the algorithm
    private readonly float k2; // Constants used in the algorithm

    private readonly float k3; // Constants used in the algorithm
    ...
    public Vector3? Update(float T, Vector3 x, Vector3? xd = null)
    {
        ...
        else
        {
            ...
            var t1 = Mathf.Exp(-_z * _w * T);
            var alpha = 2 * t1 * (_z <= 1 ? Mathf.Cos(T * _d) : (float)Math.Cosh(T * _d));
            var beta = t1 * t1;
...
public class CompTNS_Centrifuge : Comp_TiberiumNetworkStructure
{
    ...
    {
        new DefValue<NetworkValueDef, double>(Defs.TiberiumDefOf.Phosphorus, 1f / 8),
        new DefValue<NetworkValueDef, double>(Defs.TiberiumDefOf.Carbon, 1f / 4),
        new DefValue<NetworkValueDef, double>(Defs.TiberiumDefOf.Iron, 1f / 4),
        new DefValue<NetworkValueDef, double>(Defs.TiberiumDefOf.Calcium, 1f / 8),
        new DefValue<NetworkValueDef, double>(Defs.TiberiumDefOf.Copper, 1f / 4),
        new DefValue<NetworkValueDef, double>(Defs.TiberiumDefOf.Silicon, 1f / 8),
        new DefValue<NetworkValueDef, double>(Defs.TiberiumDefOf.Exotic, 1f / 16)
    };
    ...
    private static readonly List<DefValue<NetworkValueDef, double>> BlueTibValues = new()
    {
        ...
        new DefValue<NetworkValueDef, double>(Defs.TiberiumDefOf.Silicon, 1f / 8),
        new DefValue<NetworkValueDef, double>(Defs.TiberiumDefOf.Exotic, 1f / 16),
        new DefValue<NetworkValueDef, double>(Defs.TiberiumDefOf.Silver, 1f / 8)
    };
    ...
    private static readonly List<DefValue<NetworkValueDef, double>> RedTibValues = new()
    {
        ...
        new DefValue<NetworkValueDef, double>(Defs.TiberiumDefOf.Silicon, 1f / 8),
        new DefValue<NetworkValueDef, double>(Defs.TiberiumDefOf.Exotic, 1f / 16),
        new DefValue<NetworkValueDef, double>(Defs.TiberiumDefOf.Gold, 1f / 8),
        new DefValue<NetworkValueDef, double>(Defs.TiberiumDefOf.Uranium, 1f / 8)
    };

    private readonly SimpleCurve shaderCurve = new()
    {
        new CurvePoint(0, 0),
    ...
    private INetworkPart ChemicalComponent => this[Defs.TiberiumDefOf.ChemicalNetwork];

    private float BlendValue =>
        OverrideBlendValue > 0 ? OverrideBlendValue : shaderCurve.Evaluate(speedController.CurPct);
    ...
    public override void NetworkPostTick(INetworkPart networkSubPart, bool isPowered)
    {
        ...
        if (speedController.ReachedPeak && processingBatch)
        {
            ...
            for (var i = storedTypes.Count - 1; i >= 0; i--)
            {
                ...
                if (TiberiumNetPart.Volume.TryRemove(storedType, 1f, out var result))
                    foreach (var type in values)
                    {
                        ...
                        WasteNetPart.Volume.TryAdd(Defs.TiberiumDefOf.TibSludge, 0.125f, out _);
                    }
    ...
    {
        if (def == Defs.TiberiumDefOf.TibGreen)
            return GreenTibValues;
        if (def == Defs.TiberiumDefOf.TibBlue)
            return BlueTibValues;
        if (def == Defs.TiberiumDefOf.TibRed)
            return RedTibValues;
