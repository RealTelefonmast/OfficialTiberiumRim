...
public class IncidentWorker_ChronoAnomaly : IncidentWorker
{
    ...
    protected override bool TryExecuteWorker(IncidentParms parms)
    {
        ...
        if (!DropCellFinder.TryFindRaidDropCenterClose(out spawnLoc, map, false, false))
            spawnLoc = DropCellFinder.FindRaidDropCenterDistant_NewTemp(map, false);

        var composition = new ActionComposition("VolkovArrival");
        composition.AddPart(delegate { TRUtils.CameraPanNLock().PanDirect(spawnLoc, 5); }, 0);
        composition.AddPart(delegate
        {
            var Vortex = (ChronoVortex)ThingMaker.MakeThing(RedAlertDefOf.ChronoVortexPortal);
