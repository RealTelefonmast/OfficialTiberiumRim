...
namespace TR.ThingData.BeamHub;

public class Building_BeamHub : TRBuildingPrototype
{
    private bool[] allowedDirections = new[] { true, true, true, true };
    private List<Building_BeamHub> connectedHubs = new(4) { null, null, null, null };

    private List<ScribeList<Building_BeamHubSegmentPart>> connectedSegments = new(4)
    {
    ...
    private void TryFindHubs()
    {
        ...
        for (var i = 0; i < 4; i++) LookForHub(i);
    }
    ...
    private void LookForHub(int direction)
    {
        ...
        for (var i = 0; i <= def.beamHub.range; i++)
        {
            ...
            if (thing != null && thing != this &&
                !(thing is Building_BeamHub || thing is Building_BeamHubSegmentPart)) break;
            cells.Add(curCell);
    ...
    private void MakeSegments(Building_BeamHub toOther, List<IntVec3> cells, int direction)
    {
        foreach (var cell in cells)
        {
            ...
            if (cell.GetFirstThing(Map, def.beamHub.segmentDef) is Building_BeamHubSegmentPart part)
                //Existing segment found, update new parents
                segmentPart = part;
            else
                //Make new segment, add connection
                segmentPart = (Building_BeamHubSegmentPart)GenSpawn.Spawn(def.beamHub.segmentDef, cell, Map);
    ...
    public override string GetInspectString()
    {
        ...
        sb.AppendLine("\nAllowed: " + allowedDirections[0] + ", " + allowedDirections[1] + ", " + allowedDirections[2] +
                      ", " + allowedDirections[3] + "\n");
        sb.AppendLine("Hubs: " + (connectedHubs[0] != null) + ", " + (connectedHubs[1] != null) + ", " +
                      (connectedHubs[2] != null) + ", " + (connectedHubs[3] != null) + "\n");
    ...
    public override void DrawAt(Vector3 drawLoc, bool flip = false)
    {
        ...
        foreach (var hub in connectedHubs.Where(hub => hub != null)) GenDraw.DrawLineBetween(DrawPos, hub.DrawPos);
    }
