...
public static class GenTiberium
{
    public static bool TrySpreadTiberium(TiberiumCrystal crystal)
    {
        ...
        {
            return TrySpawnTiberium(c, crystal.Map, crystalDef, crystal.Parent);
        }
    ...
    {
        if (cell.HasTiberium(map) || cell.HasTibFlora(map)) return false;
        return true;
    }
    ...
    {
        requiresFlora = false;
        if (!cell.CanGrowTo(map)) return false;

        //Flora Check
        if (ShouldGrowFloraAt(cell, map))
        {
            if (cell.HasTibFlora(map)) return false;
            requiresFlora = true;
            return false;
        }
    ...
    public static bool TrySpawnTiberium(IntVec3 pos, Map map, TiberiumCrystalDef def, TiberiumProducer parent = null)
    {
        ...
        if (!def.TryGetTiberiumOutcomesAt(pos, map, out var topTerrain, out var underTerrain, out var crystalDef))
            return false;

        //Check if the tiberium can spread to the position
        if (!AllowsTiberiumAt(pos, map, def, out var requiresFlora))
        {
            if (requiresFlora && pos.Standable(map)) GenSpawn.Spawn(TiberiumDefOf.TiberiumGrass, pos, map);
            return false;
        }
    ...
    public static TiberiumCrystal SpawnTiberium(IntVec3 pos, Map map, TiberiumCrystalDef def,
        TiberiumProducer parent = null, bool withTerrain = false)
    {
        if (pos.GetTiberium(map) != null) return null;
        def.GetTiberiumOutcomesAt(pos, map, out var topTerrain, out var underTerrain, out var crystalDef);
        if (withTerrain)
            SetTerrain(pos, map, topTerrain, underTerrain);
    ...
    public static TiberiumCrystal Spawn(IntVec3 pos, Map map, TiberiumCrystalDef def, TiberiumProducer parent = null)
    {
        ...
        if (parent != null)
            newCrystal.PreSpawnSetup(parent);
    ...
    public static bool TryMutatePlant(Plant plant, TiberiumCrystalDef def)
    {
        ...
        var hasOut = def.GetFloraOutcomes(plant.def, out var toPlant, out var floraTerrain);
    ...
    //Tiberium Spores
    public static BlossomSpore SpawnBlossomSpore(IntVec3 start, IntVec3 dest, Map map, TiberiumProducerDef blossom,
        TiberiumProducer parent)
    {
        var spore = (BlossomSpore)ParticleMaker.MakeParticle(TiberiumDefOf.BlossomSpore);
    ...
    public static TiberiumSpore SpawnSpore(CellRect startRect, IntVec3 dest, Map map, TiberiumCrystalDef type,
        TiberiumProducer parent, bool dustlike = false)
    {
        if (type == null)
            return null;
        var cell = startRect.Cells.RandomElement();
        return SpawnSpore(cell, dest, map, type, parent, dustlike);
    }

    public static TiberiumSpore SpawnSpore(IntVec3 start, IntVec3 dest, Map map, TiberiumCrystalDef type,
        TiberiumProducer parent, bool dustlike = false)
    {
        if (type == null)
            return null;
        var def = dustlike ? TiberiumDefOf.TiberiumDustSpore : TiberiumDefOf.TiberiumSpore;
        var spore = (TiberiumSpore)ParticleMaker.MakeParticle(def);
        start += parent?.def.spawner.sporeOffset ?? IntVec3.Zero;
        spore.SporeSetup(type, parent);
        return (TiberiumSpore)ParticleMaker.SpawnParticleWithPath(start, dest, map, spore);
    }

    public static void SpawnSpore(CellRect startRect, float radius, Map map, TiberiumCrystalDef type,
        TiberiumProducer parent, int sporeCount = 1, bool dustlike = false)
    {
        if (type == null)
            return;
        var def = dustlike
            ? DefDatabase<ParticleDef>.GetNamed("TiberiumDustSpore")
            : DefDatabase<ParticleDef>.GetNamed("TiberiumSpore");
        var alreadyAdded = new List<IntVec3>();
        for (var i = 0; i < sporeCount; i++)
        {
            ...
            CellFinder.TryFindRandomCellNear(cell, map, (int)radius,
                c => !alreadyAdded.Contains(c) && c.InBounds(map) && !c.Fogged(map) && !c.Roofed(map) &&
                     !c.GetTerrain(map).IsWater && c.Standable(map) && c.GetTiberium(map) == null, out var dest);
    ...
    {
        return c.InBounds(map) && c.GetTerrain(map) is TerrainDef terrain && !terrain.HasTag("Water") &&
               !terrain.IsStone();
    }

    public static bool SupportsBlossom(this IntVec3 c, Map map)
    {
        return c.InBounds(map) && !c.Fogged(map) && !c.Roofed(map) && c.Standable(map) &&
               TiberiumDefOf.TerrainFilter_Soil.Allows(c.GetTerrain(map));
    }
