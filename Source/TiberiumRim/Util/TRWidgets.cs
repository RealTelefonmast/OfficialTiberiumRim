...
public static class TRWidgets
{
    ...
    public static void AddGapLine(ref float curY, float width, float gapSize, float x = 0,
    ...
    public static void DrawTexture(float x, float y, float width, Texture2D texture, out float height)
    {
        ...
        dimensions /= mainPct;
        height = dimensions.y;
        var rect = new Rect(x, y, dimensions.x, dimensions.y);
    ...
    public static bool ButtonColoredHighlight(Rect rect, string label, Color bgColor, bool mouseOverSound = true,
        int thickness = 1)
    {
    ...
    public static bool ButtonColoredHighlight(Rect rect, string label, Color bgColor, Color borderColor,
        bool mouseOverSound = true, int thickness = 1)
    {
    ...
    public static void DrawTiberiumTypeReadout(Rect rect, GameFont font, float textYOffset,
        Dictionary<TiberiumValueType, float> typeValues, out float height)
    {
        GUI.BeginGroup(rect);
        height = 5;
        Text.Font = font;
        ...
        foreach (var type in typeValues.Keys)
        {
            ...
            //if(value <= 0) continue;
            var label = type + ": " + typeValues[type];
            var typeRect = new Rect(5, height, 10, 10);
            var typeSize = Text.CalcSize(label);
            var typeLabelRect = new Rect(20, height + textYOffset, typeSize.x, typeSize.y);
