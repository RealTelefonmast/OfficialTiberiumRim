...
public static class Geometry
{
    ...
    {
        var determinant = (a.x - p.x) * (b.y - p.y) - (a.y - p.y) * (b.x - p.x);
    ...
    public static float DistanceFromPointToPlane(Vector3 planeNormal, Vector3 planePos, Vector3 pointPos)
    {
        ...
        var distance = Vector3.Dot(planeNormal, pointPos - planePos);

        return distance;
    }

    //Get the coordinate if we know a ray-plane is intersecting
    public static Vector3 GetRayPlaneIntersectionCoordinate(Vector3 planePos, Vector3 planeNormal, Vector3 rayStart,
        Vector3 rayDir)
    {
    ...
    public static bool AreLinePlaneIntersecting(Vector3 planeNormal, Vector3 planePos, Vector3 linePos1,
        Vector3 linePos2)
    {
        var areIntersecting = false;

        var lineDir = (linePos1 - linePos2).normalized;
        ...
        if (denominator > 0.000001f || denominator < -0.000001f)
        {
            ...
            if (IsPointBetweenPoints(linePos1, linePos2, intersectionPoint)) areIntersecting = true;
        }
    ...
    public static bool IsPointBetweenPoints(Vector3 a, Vector3 b, Vector3 c)
    {
        ...
        if (Vector3.Dot(ab, ac) > 0f && ab.sqrMagnitude >= ac.sqrMagnitude) isBetween = true;

        return isBetween;
    }

    //We know a line plane is intersecting and now we want the coordinate of intersection
    public static Vector3 GetLinePlaneIntersectionCoordinate(Vector3 planeNormal, Vector3 planePos, Vector3 linePos1,
        Vector3 linePos2)
    {
        var vecBetween = planePos - linePos1;
    ...
    public static bool IsPointInPolygon(List<Vector2> polygonPoints, Vector2 point)
    {
        ...
        for (var i = 0; i < polygonPoints.Count; i++)
        {
            ...
            var iPlusOne = ClampListIndex(i + 1, polygonPoints.Count);

            var l2_p2 = polygonPoints[iPlusOne];

            //Are the lines intersecting?
            if (AreLinesIntersecting(l1_p1, l1_p2, l2_p1, l2_p2, true)) numberOfIntersections += 1;
        }
        ...
        if (numberOfIntersections == 0 || numberOfIntersections % 2 == 0) isInside = false;
